...
import { compileFieldRevisions, validateSourceNativeObjectMap } from '../source/object-map.js';
import type {
  SourceNativeField,
      ...
      return [
        {
          ...
          validAtMs,
          stateClassKey: sourceNativeFieldStateSha256(field),
        },
      ];
    })
...
function expectedRevisionCensus(map: SourceNativeObjectMap): {
  ...
  try {
    ...
    return {
      revisions,
      complete:
        revisions.length === map.fieldRevisions.length &&
...
function emptyResult({
  ...
  observationClosure = [],
  revisionClosure = [],
  applicableRevisionHashes = [],
  activeSupersessionIds = [],
  derivedValidAtFieldCount = 0,
  ambiguous = false,
}: {
  ...
  const core = {
    ...
    observationClosureSha256: stableObjectSha256(
      observationClosure.map((row) => row.nativeObjectSha256),
    ),
    ...
    complete:
      controls.mappedCoverageComplete &&
      controls.adapterDiagnosticsClear &&
...
export function resolveSourceNativeFieldAt({
  ...
  const controls = {
    mappedCoverageComplete:
      map.mappedSourceCount === map.sourceCount && map.unsupportedSourceCount === 0,
    adapterDiagnosticsClear: map.parseFailureCount === 0 && map.adapterDiagnostics.length === 0,
  ...
  if (identityIds.length !== 1) {
    return emptyResult({
      ...
      controls,
      ambiguous: identityIds.length > 1,
    });
  }
  const objectIdentitySha256 = identityIds[0]!;
  const selectedIdentity = scopedMatches.find(
    (object) => object.objectIdentitySha256 === objectIdentitySha256,
  )!.objectIdentity;
  resolvedQuery.externalId = selectedIdentity.externalId;
  const identityObjects = map.nativeObjects.filter(
    (object) => object.objectIdentitySha256 === objectIdentitySha256,
  );
  ...
  const revisionClosure = map.fieldRevisions
    .filter(
      (revision) =>
        revision.objectIdentitySha256 === objectIdentitySha256 &&
  ...
  const sourceObservedThrough = map.nativeObjects.reduce<string | null>((latest, object) => {
    if (latest === null || Date.parse(object.occurredAt) > Date.parse(latest))
      return object.occurredAt;
    return latest;
  }, null);
  const observationClosure = observations.map((observation) => observation.object);
  const derivedValidAtFieldCount = observations.filter(
    (observation) => observation.field.validAt === undefined,
  ).length;
  const atMs = Date.parse(at);
  const firstValidAtMs = observations.reduce(
    (earliest, observation) => Math.min(earliest, observation.validAtMs),
  ...
  if (sourceObservedThrough !== null && atMs > Date.parse(sourceObservedThrough)) {
    return emptyResult({
      ...
      query: resolvedQuery,
      at,
      state: 'unavailable-native-historical-field-beyond-source-horizon',
      controls,
      sourceObservedThrough,
      observationClosure,
      revisionClosure,
  ...
  if (atMs < firstValidAtMs) {
    return emptyResult({
      ...
      query: resolvedQuery,
      at,
      state: 'unavailable-native-historical-field-not-yet-valid',
      controls,
      sourceObservedThrough,
      observationClosure,
      revisionClosure,
  ...
  for (const candidate of classes) {
    for (const observation of candidate.observations)
      classByObservation.set(observation, candidate);
  }
  const observationByEndpoint = new Map(
    observations.map((observation) => [
      fieldEndpointKey(observation.object.relativePath, observation.field),
      observation,
    ]),
  );
  ...
  const applicableRevisions = revisionClosure.filter((revision) => {
    const source = observationByEndpoint.get(
      fieldEndpointKey(revision.sourceField.evidence.relativePath, revision.sourceField),
    );
    const target = observationByEndpoint.get(
      fieldEndpointKey(revision.targetField.evidence.relativePath, revision.targetField),
    );
    return (
      source !== undefined &&
      target !== undefined &&
      source.validAtMs <= atMs &&
      target.validAtMs <= atMs
    );
  });
  ...
  for (const revision of applicableRevisions) {
    const sourceObservation = observationByEndpoint.get(
      fieldEndpointKey(revision.sourceField.evidence.relativePath, revision.sourceField),
    );
    const targetObservation = observationByEndpoint.get(
      fieldEndpointKey(revision.targetField.evidence.relativePath, revision.targetField),
    );
  ...
  if (survivingClasses.length !== 1) {
    return emptyResult({
      ...
      query: resolvedQuery,
      at,
      state: 'unavailable-native-historical-field-ambiguous',
      controls,
      sourceObservedThrough,
      observationClosure,
      revisionClosure,
  ...
  const selectedObservation = selectedClass.observations
    .filter((observation) => observation.validAtMs <= atMs)
    .sort(compareObservation)
    .at(-1);
