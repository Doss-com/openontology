...
export function validateSourceNativeExactSourceAvailabilitySnapshot(
  value: UnknownRecord,
  {
    sourceCommitSha256,
    sourceReplaySha256,
  ...
  if (
    value?.kind !== 'OpenOntologyExactSourceAvailabilitySnapshotV1' ||
    !SHA256.test(snapshot) ||
    stableObjectSha256(core) !== snapshot ||
...
export function compileSourceNativeExactSourceAvailabilitySnapshot({
  ...
  const unavailableSourceMessageIds = [...unavailableInput].sort(
    (left: number, right: number) => left - right,
  );
  if (
    new Set(unavailableSourceMessageIds).size !== unavailableSourceMessageIds.length ||
...
export function openSourceNativeExactEvidenceSession(options: ExactSessionOptions) {
  ...
  const tokenizeFn =
    typeof tokenize === 'function' ? tokenize : fail('SOURCE_NATIVE_EXACT_SESSION_INPUT');
  if (
    typeof namespace !== 'string' ||
    !namespace ||
    !SHA256.test(mapSha256) ||
    sourceRows.length < 1 ||
    typeof tokenizeFn !== 'function' ||
    typeof retrievalAdapter !== 'string' ||
    !retrievalAdapter ||
    !SHA256.test(retrievalSha256) ||
    !Number.isSafeInteger(maximumSearchResults) ||
  ...
  const sources = sourceRows
    ...
    .sort(
      (left: ExactSessionSource, right: ExactSessionSource) =>
        left.ordinal - right.ordinal ||
        left.sourceMessageId - right.sourceMessageId ||
        compare(left.relativePath, right.relativePath),
    );
  if (
    new Set(sources.map((row: ExactSessionSource) => row.sourceMessageId)).size !==
      sources.length ||
  ...
  const sourceById = new Map<number, ExactSessionSource>(
    sources.map((row: ExactSessionSource) => [row.sourceMessageId, row]),
  );
  const sourceByRef = new Map<string, ExactSessionSource>(
  ...
  if (
    ...
    (objectOnt?.kind !== 'OpenOntologySourceNativeObjectOntModuleV1' ||
      ...
      sources.some(
        (source: ExactSessionSource) =>
          !objectOnt.sources.some(
            (bound: UnknownRecord) =>
  ...
  const sourceHandles = freeze(
    sources.map((row: ExactSessionSource) =>
      freeze({
        sourceMessageId: row.sourceMessageId,
        relativePath: row.relativePath,
      }),
  ...
  const unavailableSourceIds = new Set<number>(
    exactSourceAvailabilitySnapshot.unavailableSourceMessageIds,
  );
  ...
  const exactSource = (source: ExactSessionSource): UnknownRecord =>
    freeze({
  ...
  const inspectEnvelope = (toolId: string, request: UnknownRecord, rows: ExactSessionSource[]) => {
    ...
    const ref =
      toolId === 'inspect' ? request.ref : `sources:${stableObjectSha256(request.sources)}`;
    const textValue = {
      schemaVersion: 1,
  ...
  const search = async (request: SearchRequest) => {
    ...
    const ranked = index.rank(request.query, {
      limit: Math.min(request.limit, maximumSearchResults),
    });
    const candidates = freeze(
      ranked.map((row: { id: string; score: number }, rank: number) => {
  ...
  const inspect = async (request: UnknownRecord & { ref?: unknown }) => {
    ...
    const ref =
      typeof request.ref === 'string' ? request.ref : fail('SOURCE_NATIVE_EXACT_SESSION_INSPECT');
    const source = sourceByRef.get(ref);
    const exact: ExactSessionSource =
      source && !unavailableSourceIds.has(source.sourceMessageId)
        ? source
        : fail('SOURCE_NATIVE_EXACT_SESSION_INSPECT');
    return inspectEnvelope('inspect', request, [exact]);
  };
  const offerNavigationSources = (
    ...
    const selectionSha256 =
      typeof request.selectionSha256 === 'string' ? request.selectionSha256 : '';
    if (
      typeof request?.selectionKind !== 'string' ||
      ...
      sourceMessageIds.length > 128 ||
      new Set(sourceMessageIds).size !== sourceMessageIds.length ||
      sourceMessageIds.some(
        (sourceMessageId: number) =>
          !sourceById.has(sourceMessageId) || unavailableSourceIds.has(sourceMessageId),
      )
    ) {
  ...
  const inspectSources = async (
    request: UnknownRecord & {
      sources?: Array<UnknownRecord & { sourceMessageId?: unknown; navigationQuery?: unknown }>;
    },
  ) => {
    const sourceRequests = request.sources ?? [];
    if (
      sourceRequests.length < 1 ||
      sourceRequests.length > 32 ||
      new Set(sourceRequests.map((row) => row?.sourceMessageId)).size !== sourceRequests.length ||
      sourceRequests.some(
        (row) =>
          !Number.isSafeInteger(row?.sourceMessageId) ||
          !offeredSourceMessageIds.has(
            typeof row.sourceMessageId === 'number' ? row.sourceMessageId : -1,
          ) ||
          typeof row.navigationQuery !== 'string' ||
          !row.navigationQuery.trim() ||
    ...
    const rows = requestedSourceMessageIds
      .map((sourceMessageId) => sourceById.get(sourceMessageId))
