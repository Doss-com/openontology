  ...
  return freeze(
    schemasInput
      .map((value: unknown) => {
        ...
        const aliasesInput: unknown[] = Array.isArray(schema.aliases)
          ? schema.aliases
          : fail('SOURCE_NATIVE_QUERY_SCHEMA');
        const fieldsInput: unknown[] = Array.isArray(schema.fields)
          ? schema.fields
          : fail('SOURCE_NATIVE_QUERY_SCHEMA');
        ...
        if (aliases.length !== aliasesInput.length || aliases.some((alias) => !alias))
          fail('SOURCE_NATIVE_QUERY_SCHEMA');
        const fieldPaths = new Set<string>();
        const fields = fieldsInput
          .map((fieldValue: unknown) => {
            if (!fieldValue || typeof fieldValue !== 'object' || Array.isArray(fieldValue))
              fail('SOURCE_NATIVE_QUERY_SCHEMA');
            const field = fieldValue as Record<string, unknown>;
            const fieldPath = typeof field.fieldPath === 'string' ? field.fieldPath : '';
            const fieldAliasesInput: unknown[] = Array.isArray(field.aliases)
              ? field.aliases
              : fail('SOURCE_NATIVE_QUERY_SCHEMA');
            if (!fieldPath || fieldPaths.has(fieldPath) || fieldAliasesInput.length < 1)
              fail('SOURCE_NATIVE_QUERY_SCHEMA');
            fieldPaths.add(fieldPath);
            const fieldAliases = [...new Set(fieldAliasesInput.map(normalized))].sort();
            if (
              fieldAliases.length !== fieldAliasesInput.length ||
              fieldAliases.some((alias) => !alias)
            ) {
              fail('SOURCE_NATIVE_QUERY_SCHEMA');
            }
...
export function compileSourceNativeFieldQuery({
  question,
  schemas: schemaInput,
  typedQuery = null,
}: QueryPlanInput = {}): CompiledSourceNativeFieldQueryPlan {
  ...
  const scopedObject =
    typedQuery === null
      ...
      : objects.find(
          ({ schema }) =>
            schema.sourceSystem === typedQuery.sourceSystem &&
            schema.objectType === typedQuery.objectType,
        );
