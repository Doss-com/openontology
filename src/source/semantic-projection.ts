...
export function compileSourceNativeProofAuthorityProjection({
  ...
  const rootPropositionKeys =
    rootPropositionKeyInput === undefined ? null : [...rootPropositionKeyInput].sort(compare);
  ...
  for (const object of map.nativeObjects) {
    ...
    for (const field of object.fields) {
      ...
      relations.push(
        ...proposition.relations.map((relation) => ({
          type: relation.type,
          sourceProjectionItemId: proposition.propositionKey,
  ...
  if (at !== null && itemIds.size !== bindings.length) {
    ...
    for (const group of byKey.values()) {
      ...
      if (
        group.some(
          (row) =>
            row.objectIdentitySha256 !== first.objectIdentitySha256 ||
      ...
      const observations = map.nativeObjects
        ...
        .flatMap((object) =>
          object.fields.filter((field) => field.fieldPath === first.field.fieldPath),
        );
      ...
      const lastIndex = observations.reduce(
        (last, field, index) => (repeatedFields.has(field.fieldSha256) ? index : last),
        -1,
      );
      if (
        firstIndex < 0 ||
  ...
  let selectedBindings = bindings.filter((row) => selectedIds.has(row.item.sourceProjectionItemId));
  let selectedRelations = relations.filter(
    (relation) =>
      selectedIds.has(relation.sourceProjectionItemId) &&
      selectedIds.has(relation.targetProjectionItemId),
  );
  if (at !== null) {
    ...
    if (
      activeIds.size !== selectedBindings.length ||
    ...
    selectedRelations = selectedRelations.filter(
      (relation) =>
        activeIds.has(relation.sourceProjectionItemId) &&
        activeIds.has(relation.targetProjectionItemId),
    );
    selectedRelations = [
      ...new Map(
        selectedRelations.map((relation) => [stableObjectSha256(relation), relation]),
      ).values(),
    ];
    if (rootPropositionKeys !== null) {
      ...
      while (expanded) {
        ...
        for (const relation of selectedRelations) {
          if (
            ...
            !connected.has(relation.sourceProjectionItemId)
          ) {
      ...
      selectedBindings = selectedBindings.filter((row) =>
        connected.has(row.item.sourceProjectionItemId),
      );
      selectedRelations = selectedRelations.filter(
        (relation) =>
          connected.has(relation.sourceProjectionItemId) &&
