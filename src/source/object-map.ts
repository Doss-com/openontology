...
export type SourceNativeCanonicalRole =
  'action' | 'actor' | 'change' | 'chronology' | 'counterevidence' | 'outcome' | 'state';
export type SourceNativePropositionModality =
  'observed' | 'planned' | 'reported' | 'static-only' | 'unresolved';
...
const PROPOSITION_MODALITIES = new Set([
  'observed',
  'planned',
  'reported',
  'static-only',
...
function normalizeSourceInput(value: unknown): ExactSourceInput {
  ...
  const sourceType =
    typeof row.sourceType === 'string' ? row.sourceType : fail('SOURCE_NATIVE_SOURCE');
  const relativePath =
    typeof row.relativePath === 'string' ? row.relativePath : fail('SOURCE_NATIVE_SOURCE');
  const occurredAt =
    typeof row.occurredAt === 'string' ? row.occurredAt : fail('SOURCE_NATIVE_SOURCE');
  const content = typeof row.content === 'string' ? row.content : fail('SOURCE_NATIVE_SOURCE');
  const sourceSha256 =
    typeof row.sourceSha256 === 'string' ? row.sourceSha256 : fail('SOURCE_NATIVE_SOURCE');
  return {
    sourceType,
    relativePath,
    occurredAt,
...
function normalizeFieldInput(value: unknown): ExactFieldInput {
  ...
  const propositionFamilyKey =
    row.propositionFamilyKey === undefined
      ? null
      : typeof row.propositionFamilyKey === 'string'
        ? row.propositionFamilyKey
        : fail('SOURCE_NATIVE_FIELD_INPUT');
  const businessEntityKeys =
    row.businessEntityKeys === undefined
      ? null
      : Array.isArray(row.businessEntityKeys) &&
          row.businessEntityKeys.every((key) => typeof key === 'string')
        ? row.businessEntityKeys
        : fail('SOURCE_NATIVE_FIELD_INPUT');
  const canonicalProposition =
    row.canonicalProposition === undefined
      ? null
      : isRecord(row.canonicalProposition)
        ? row.canonicalProposition
        : fail('SOURCE_NATIVE_FIELD_INPUT');
  const validAt =
    row.validAt === undefined
      ? null
      : typeof row.validAt === 'string'
        ? row.validAt
        : fail('SOURCE_NATIVE_FIELD_INPUT');
  const knownAt =
    row.knownAt === undefined
      ? null
      : typeof row.knownAt === 'string'
        ? row.knownAt
        : fail('SOURCE_NATIVE_FIELD_INPUT');
  const canonicalValue =
    row.canonicalValue === undefined
      ? null
      : isJsonValue(row.canonicalValue)
        ? row.canonicalValue
        : fail('SOURCE_NATIVE_FIELD_INPUT');
  const provenanceBy =
    row.provenanceBy === undefined
      ? null
      : isRecord(row.provenanceBy)
        ? row.provenanceBy
        : fail('SOURCE_NATIVE_FIELD_INPUT');
  const actorResolutionEvidence =
    row.actorResolutionEvidence === undefined
      ? null
      : isRecord(row.actorResolutionEvidence)
        ? row.actorResolutionEvidence
        : fail('SOURCE_NATIVE_FIELD_INPUT');
...
function validateIdentity(value: unknown): SourceNativeObjectIdentity {
  const identity = exactRecord(
    jsonClone(value, 'SOURCE_NATIVE_OBJECT_IDENTITY'),
    'SOURCE_NATIVE_OBJECT_IDENTITY',
  );
  if (
    identity.home !== 'ObjectDef/InstanceRef' ||
    ...
    fail('SOURCE_NATIVE_OBJECT_IDENTITY');
  const sourceSystem =
    typeof identity.sourceSystem === 'string'
      ? identity.sourceSystem
      : fail('SOURCE_NATIVE_OBJECT_IDENTITY');
...
function validateProvenanceBy(value: unknown): UnknownRecord | null {
  ...
  const provenanceBy = exactRecord(
    jsonClone(value, 'SOURCE_NATIVE_PROVENANCE_BY'),
    'SOURCE_NATIVE_PROVENANCE_BY',
  );
  const roleLabels = provenanceBy.roleLabels;
  if (
    provenanceBy.home !== 'ObjectDef/InstanceRef' ||
    typeof provenanceBy.sourceSystem !== 'string' ||
    !provenanceBy.sourceSystem ||
    typeof provenanceBy.displayName !== 'string' ||
    !provenanceBy.displayName.trim() ||
...
function exactActorResolutionEvidence({
  source,
  input,
  ...
  const exactBusinessEntityKeys = isStringArray(inputBusinessEntityKeys)
    ? inputBusinessEntityKeys
    : fail('SOURCE_NATIVE_ACTOR_RESOLUTION_EVIDENCE');
  const inputCodeUnitStart =
    typeof input.codeUnitStart === 'number' && Number.isSafeInteger(input.codeUnitStart)
      ? input.codeUnitStart
      : fail('SOURCE_NATIVE_ACTOR_RESOLUTION_EVIDENCE');
  const exactInputValue =
    typeof inputValue === 'string' && inputValue
      ? inputValue
      : fail('SOURCE_NATIVE_ACTOR_RESOLUTION_EVIDENCE');
  if (
    exactBusinessEntityKeys.length !== 1 ||
  ...
  if (
    businessEntityKeys.length !== 1 ||
    !fieldBusinessEntityKeys?.includes(businessEntityKeys[0]) ||
    businessEntityKeys.length !== exactBusinessEntityKeys.length
  )
    fail('SOURCE_NATIVE_ACTOR_RESOLUTION_EVIDENCE');
...
function validateCanonicalProposition(
  value: UnknownRecord,
  {
    propositionFamilyKey,
    fieldBusinessEntityKeys,
  ...
  if (
    ![
      'OpenOntologySourceNativeCanonicalPropositionV1',
      'OpenOntologySourceNativeCanonicalPropositionV2',
    ...
    (version === 'OpenOntologySourceNativeCanonicalPropositionV2' &&
      (typeof value.propositionKey !== 'string' ||
        !value.propositionKey ||
        propositionFamilyKey === null ||
        value.dimension !== propositionFamilyKey ||
        !isStringArray(canonicalRoles) ||
        canonicalRoles.length < 1 ||
...
function exactField({
  ...
  codeUnitStart,
  propositionFamilyKey = null,
  businessEntityKeys: fieldBusinessEntityKeys = null,
  canonicalProposition: canonicalPropositionInput = null,
  validAt = null,
  knownAt = null,
  canonicalValue: canonicalValueInput = null,
  provenanceBy: provenanceByInput = null,
  ...
  if (
    ...
    typeof value !== 'string' ||
    !value ||
    !Number.isSafeInteger(codeUnitStart) ||
    codeUnitStart < 0 ||
  ...
  const core: Omit<SourceNativeField, 'fieldSha256'> = {
    ...
    ...(canonicalProposition === null
      ? {}
      : { canonicalProposition: freeze(canonicalProposition) }),
...
function compileObject(input: ExactObjectInput, source: SourceNativeSource): SourceNativeObject {
  if (
    input?.relativePath !== source.relativePath ||
  ...
  if (
    ...
    duplicateEvidenceFieldPaths.some(
      (fieldPath) => !fields.some((field) => field.fieldPath === fieldPath),
    )
  ) {
    fail('SOURCE_NATIVE_DUPLICATE_FIELDS');
  }
  ...
  if (
    ...
    (typeof transportOriginSystem !== 'string' ||
      !/^[a-z][a-z0-9-]{0,63}$/u.test(transportOriginSystem) ||
  ...
  const core: Omit<SourceNativeObject, 'nativeObjectSha256'> = {
    ...
    transportOriginSystem,
    fields: freeze(
      fields.sort((left: SourceNativeField, right: SourceNativeField) =>
        compare(left.fieldPath, right.fieldPath),
      ),
    ),
  ...
  for (const rows of groups.values()) {
    ...
    for (let index = 1; index < rows.length; index += 1) {
      ...
      const canonicalComparison =
        source.field.canonicalValue !== undefined || target.field.canonicalValue !== undefined;
...
function compileDuplicateEvidenceClusters(objects: SourceNativeObject[]): UnknownRecord[] {
  ...
  >();
  for (const object of objects)
    for (const fieldPath of object.duplicateEvidenceFieldPaths) {
      ...
      const businessEntityKeySets =
        exactFieldValue.businessEntityKeys === undefined
          ? [object.businessEntityKeys]
          : exactFieldValue.businessEntityKeys.map((businessEntityKey: string) => [
              businessEntityKey,
            ]);
      for (const clusterBusinessEntityKeys of businessEntityKeySets.length > 0
        ...
        rows.push({
          object,
          field: exactFieldValue,
          propositionFamilyKey,
          clusterBusinessEntityKeys,
        });
    ...
    [...groups.entries()]
      ...
      .map(([fingerprint, rows]) => {
        ...
        const core: UnknownRecord = {
          ...
          fieldPath:
            new Set(rows.map((row) => row.field.fieldPath)).size === 1
              ? rows[0].field.fieldPath
              : null,
...
function compileBusinessEntityEvidenceNeighborhoods(
  objects: SourceNativeObject[],
): UnknownRecord[] {
  const groups = new Map<string, SourceNativeObject[]>();
  for (const object of objects)
    for (const businessEntityKey of object.businessEntityKeys) {
  ...
  return freeze(
    [...groups.entries()]
      ...
      .map(([groupKey, rows]) => {
        ...
        const propositionSignatureByPath = new Map(
          rows.map((row) => {
            const duplicateFields = row.duplicateEvidenceFieldPaths
              .map((fieldPath) =>
            ...
            const fingerprints = (
              entityScopedFields.length > 0 ? entityScopedFields : duplicateFields
            ).map((field) =>
              stableObjectSha256({
        ...
        const sameTransport = (left: SourceNativeObject, right: SourceNativeObject) =>
          left.transportOriginSystem === right.objectIdentity.sourceSystem ||
        ...
        for (let leftIndex = 0; leftIndex < rows.length; leftIndex += 1) {
          ...
          for (let rightIndex = leftIndex + 1; rightIndex < rows.length; rightIndex += 1) {
            ...
            if (
              stableObjectSha256(leftSignature) === stableObjectSha256(rightSignature) &&
        ...
        for (const paths of lineageMembers.values()) {
          paths.sort(compare);
          const evidenceLineageSha256 = stableObjectSha256({
            namespace,
            businessEntityKey,
            relativePaths: paths,
          });
          for (const relativePath of paths)
            evidenceLineageByPath.set(relativePath, evidenceLineageSha256);
        }
...
function exactNestedHash(value: unknown, field: string, code: string): void {
  ...
  if (
    ...
    !SHA256.test(observed) ||
    stableObjectSha256(core) !== observed
  )
    fail(code);
}
...
function validateCompiledField(
  field: SourceNativeField,
  object: SourceNativeObject | null = null,
): void {
  exactNestedHash(field, 'fieldSha256', 'SOURCE_NATIVE_MAP_FIELD');
  if (
    field?.schema !== 1 ||
    field.kind !== 'OpenOntologySourceNativeFieldV1' ||
    !FIELD_PATH.test(field.fieldPath ?? '') ||
    (field.propositionFamilyKey !== undefined && !FIELD_PATH.test(field.propositionFamilyKey)) ||
    typeof field.value !== 'string' ||
    !field.value ||
    !field.evidence ||
    field.evidence.relativePath !== (object?.relativePath ?? field.evidence.relativePath) ||
    (object !== null && field.evidence.sourceSha256 !== object.sourceSha256) ||
    !SHA256.test(field.evidence.sourceSha256 ?? '') ||
    !Number.isSafeInteger(field.evidence.byteStart) ||
    field.evidence.byteStart < 0 ||
    !Number.isSafeInteger(field.evidence.byteEnd) ||
    field.evidence.byteEnd <= field.evidence.byteStart ||
...
function validateCompiledObject(object: SourceNativeObject): void {
  ...
  let identity;
  try {
    identity = validateIdentity(object?.objectIdentity);
  } catch {
    fail('SOURCE_NATIVE_MAP_OBJECT');
  }
  if (
    object?.schema !== 1 ||
    object.kind !== 'OpenOntologySourceNativeObjectV1' ||
    typeof object.sourceType !== 'string' ||
    !object.sourceType ||
    logicalPath(object.relativePath, 'SOURCE_NATIVE_MAP_OBJECT').split('/')[0] !==
      object.sourceType ||
    ...
    new Set(object.duplicateEvidenceFieldPaths).size !==
      object.duplicateEvidenceFieldPaths.length ||
    object.duplicateEvidenceFieldPaths.some(
      (fieldPath) => !object.fields.some((field) => field.fieldPath === fieldPath),
    ) ||
...
function validateCompiledRevision(revision: SourceNativeFieldRevision): void {
  ...
  if (
    revision?.schema !== 1 ||
    revision.kind !== 'OpenOntologySourceNativeFieldRevisionV1' ||
    revision.relationType !== 'supersedes' ||
    !FIELD_PATH.test(revision.fieldPath ?? '') ||
    ...
    revision.navigationOnly !== true ||
    revision.exactSourcesRemainAuthority !== true
  ) {
...
export function validateSourceNativeObjectMap(value: unknown): SourceNativeObjectMap {
  ...
  if (
    ...
    map.schema !== 1 ||
    map.kind !== 'OpenOntologySourceNativeObjectMapV1'
  ) {
    fail('SOURCE_NATIVE_MAP');
  }
  if (
    !Array.isArray(map.nativeObjects) ||
    !Array.isArray(map.fieldRevisions) ||
    !Array.isArray(map.duplicateEvidenceClusters) ||
    !Array.isArray(map.businessEntityEvidenceNeighborhoods) ||
    !Array.isArray(map.adapterDiagnostics)
  )
    fail('SOURCE_NATIVE_MAP');
  const nativeObjectMapSha256 =
    typeof map.nativeObjectMapSha256 === 'string' ? map.nativeObjectMapSha256 : '';
  ...
  const unsupportedSourceCount =
    typeof map.unsupportedSourceCount === 'number' ? map.unsupportedSourceCount : -1;
  const fieldRevisionCount =
    typeof map.fieldRevisionCount === 'number' ? map.fieldRevisionCount : -1;
  const duplicateEvidenceClusterCount =
    typeof map.duplicateEvidenceClusterCount === 'number' ? map.duplicateEvidenceClusterCount : -1;
  const businessEntityEvidenceNeighborhoodCount =
    typeof map.businessEntityEvidenceNeighborhoodCount === 'number'
      ? map.businessEntityEvidenceNeighborhoodCount
      : -1;
  ...
  const duplicateEvidenceClusterValues: unknown[] = Array.isArray(map.duplicateEvidenceClusters)
    ? map.duplicateEvidenceClusters
    : fail('SOURCE_NATIVE_MAP');
  const businessEntityEvidenceNeighborhoodValues: unknown[] = Array.isArray(
    map.businessEntityEvidenceNeighborhoods,
  )
    ? map.businessEntityEvidenceNeighborhoods
    : fail('SOURCE_NATIVE_MAP');
  const adapterDiagnosticValues: unknown[] = Array.isArray(map.adapterDiagnostics)
    ? map.adapterDiagnostics
    : fail('SOURCE_NATIVE_MAP');
  if (
    !nativeObjectValues.every(isNativeObject) ||
    ...
    !adapterDiagnosticValues.every(isJsonObject)
  )
    fail('SOURCE_NATIVE_MAP');
  ...
  const businessEntityEvidenceNeighborhoods =
    businessEntityEvidenceNeighborhoodValues.filter(isRecord);
  const adapterDiagnostics = adapterDiagnosticValues.filter(isJsonObject);
  const objectIdentityHome =
    map.objectIdentityHome === 'ObjectDef/InstanceRef-plus-provenance-by'
      ? map.objectIdentityHome
      : fail('SOURCE_NATIVE_MAP');
  const propositionHome =
    map.propositionHome === 'Claim/PropositionRevision-payload'
      ? map.propositionHome
      : fail('SOURCE_NATIVE_MAP');
  const relationHome =
    map.relationHome === 'typed-relation-between-proposition-revisions'
      ? map.relationHome
      : fail('SOURCE_NATIVE_MAP');
  const exactSupportHome =
    map.exactSupportHome === 'evidence-references-and-dependencies'
      ? map.exactSupportHome
      : fail('SOURCE_NATIVE_MAP');
  ...
  if (
    ...
    stableObjectSha256(core) !== observedHash ||
    ...
    map.modelCalls !== 0 ||
    map.networkCalls !== 0 ||
    map.questionIndependent !== true ||
    map.targetLeakage !== false ||
    map.navigationOnly !== true ||
    map.exactSourcesRemainAuthority !== true
  )
    fail('SOURCE_NATIVE_MAP');
  ...
  for (const cluster of duplicateEvidenceClusters)
    exactNestedHash(cluster, 'clusterSha256', 'SOURCE_NATIVE_MAP_CLUSTER');
...
export function compileSourceNativeObjectMap({
  sources,
  nativeObjectInputs,
  adapterDiagnostics = [],
}: {
  ...
  if (
    ...
    sources.length < 1 ||
    !Array.isArray(nativeObjectInputs) ||
    !Array.isArray(adapterDiagnostics)
  )
    fail('SOURCE_NATIVE_INPUT');
  const sourceRows: unknown[] = Array.isArray(sources) ? sources : fail('SOURCE_NATIVE_INPUT');
  const objectRows: unknown[] = Array.isArray(nativeObjectInputs)
    ? nativeObjectInputs
    : fail('SOURCE_NATIVE_INPUT');
  const diagnosticRows: unknown[] = Array.isArray(adapterDiagnostics)
    ? adapterDiagnostics
    : fail('SOURCE_NATIVE_INPUT');
  ...
  const nativeObjects = objectRows
    .map((value: unknown) => {
  ...
  const businessEntityEvidenceNeighborhoods =
    compileBusinessEntityEvidenceNeighborhoods(nativeObjects);
  ...
  return validateSourceNativeObjectMap({
    ...core,
    nativeObjectMapSha256: stableObjectSha256(core),
  });
}
...
interface ObjectBackend {
  ...
  putIfAbsent(
    key: string,
    bytes: Buffer,
  ): { checksumSha256: string; version: string; replayed: boolean };
}
function validateBackend(
  backend: ObjectBackend | undefined,
  write = false,
): asserts backend is ObjectBackend {
  if (
    !backend ||
    typeof backend.get !== 'function' ||
    (write && typeof backend.putIfAbsent !== 'function')
  ) {
    fail('SOURCE_NATIVE_BACKEND');
  }
}

export function materializeSourceNativeObjectMap({
  backend,
  ...input
}: {
...
export function openSourceNativeObjectMap({
  backend,
  nativeObjectMapSha256,
}: {
  backend?: ObjectBackend;
  ...
  if (loaded.checksumSha256 !== mapHash || objectBytesSha256(loaded.bytes) !== mapHash)
    fail('SOURCE_NATIVE_READ');
  let core: unknown;
  try {
    core = JSON.parse(loaded.bytes.toString('utf8'));
  } catch {
    fail('SOURCE_NATIVE_READ');
  }
  ...
  return freeze({
    kind: 'OpenOntologySourceNativeObjectMapModuleV1',
    map,
    key,
