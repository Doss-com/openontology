...
export function resolveSourceNativeField({ sourceNativeObjectMap: mapInput,
  ...
  if (!Array.isArray(seedInput)
    ...
    || typeof query?.sourceSystem !== 'string' || !query.sourceSystem
  ...
  const supersededVersion = seededSupersededVersion ?? versions.filter((row) =>
    row.object.relativePath !== latest.object.relativePath
    && row.field.value !== latest.field.value).at(-1) ?? null;
  const revisionPath = supersededVersion === null ? [] : [freeze((() => {
  ...
  const suppressedClosure = [...new Set(versions.filter((row) => row.object.relativePath !== latest.object.relativePath
    && row.field.value !== latest.field.value).map((row) => row.object.relativePath))].sort();
  const suppressedRelativePaths = suppressedClosure.filter((relativePath) => seedRelativePaths.includes(relativePath));
  const core = resultCore({ map, query: resolvedQuery, seedRelativePaths, state: 'resolved-current-field', current,
    expandedRelativePaths, suppressedRelativePaths, revisionPath,
    revisionClosureCount: revisionClosure.length, revisionClosureSha256,
    suppressedClosureCount: suppressedClosure.length,
    suppressedClosureSha256: stableObjectSha256(suppressedClosure), policy });
  return freeze({ ...core, resolutionSha256: stableObjectSha256(core) });
}
...
export function resolveSourceNativeFieldSuccessor({ sourceNativeObjectMap: mapInput,
  ...
  const matchingSeeds = map.nativeObjects.filter((row) => seedRelativePaths.includes(row.relativePath)
    && row.objectIdentity.sourceSystem === query.sourceSystem
    && row.objectIdentity.objectType === query.objectType
    && row.objectIdentity.externalId === query.externalId
    && (query.namespace === undefined || row.objectIdentity.namespace === query.namespace)
    && row.fields.some((field) => field.fieldPath === query.fieldPath));
  const scopedMatches = query.anchorFieldSha256 === undefined ? [] : map.nativeObjects.filter((row) =>
    row.objectIdentity.sourceSystem === query.sourceSystem
    && row.objectIdentity.objectType === query.objectType
    && row.objectIdentity.externalId === query.externalId
    && (query.namespace === undefined || row.objectIdentity.namespace === query.namespace));
  const identityIds = new Set((query.anchorFieldSha256 === undefined ? matchingSeeds : scopedMatches)
    .map((row) => row.objectIdentitySha256));
  const resolvedQuery = freeze({ ...query });
  if (identityIds.size !== 1) {
    const core = successorResultCore({ map, query: resolvedQuery, seedRelativePaths,
      state: identityIds.size > 1 ? 'unavailable-native-object-seed-ambiguous'
        : 'unavailable-native-object-not-seeded' });
    return freeze({ ...core, resolutionSha256: stableObjectSha256(core) });
  }
  const objectIdentitySha256 = [...identityIds][0];
  const seedFieldByPath = new Map(matchingSeeds.map((object) => [object.relativePath,
    object.fields.find((field) => field.fieldPath === query.fieldPath)]));
  const revisions = map.fieldRevisions.filter((row) => row.objectIdentitySha256 === objectIdentitySha256
  ...
  const policy = query.anchorFieldSha256 === undefined
  ...
  if (revisions.length !== 1) {
    const core = successorResultCore({ map, query: resolvedQuery, seedRelativePaths,
      state: revisions.length > 1 ? 'unavailable-native-field-successor-ambiguous'
        : 'unavailable-native-field-successor-not-present', policy });
    return freeze({ ...core, resolutionSha256: stableObjectSha256(core) });
  }
  ...
  const successorObjects = map.nativeObjects.filter((object) =>
    object.objectIdentitySha256 === objectIdentitySha256
    && object.occurredAt === revision.sourceOccurredAt
    && object.fields.some((field) => field.fieldPath === query.fieldPath
  ...
  const proofRelativePaths = [...new Set(successorObjects.map((row) => row.relativePath))].sort(compare);
  ...
  const transition = freeze({ ...transitionCore, transitionSha256: stableObjectSha256(transitionCore) });
  const expandedRelativePaths = proofRelativePaths.filter((relativePath) =>
    !seedRelativePaths.includes(relativePath));
  const core = successorResultCore({ map, query: resolvedQuery, seedRelativePaths,
    state: 'resolved-next-field-revision', anchor, successor, proofRelativePaths,
    expandedRelativePaths, revisionPath: [transition], policy });
  return freeze({ ...core, resolutionSha256: stableObjectSha256(core) });
}
