...
function sameIdentity(left: SourceNativeObjectIdentity, right: SourceNativeObjectIdentity): boolean {
...
export function compileSourceNativeCurrentFieldChronologyVerification({
  ...
  const map = (() => {
    try { return validateSourceNativeObjectMap(mapInput); } catch {
  ...
  const sourceRows = sourceHandleInput
    ?? fail('SOURCE_NATIVE_CURRENT_FIELD_CHRONOLOGY_INPUT');
  const sourceHandles = sourceRows.map((row) => {
  ...
  if (query.externalId === undefined || query.externalId !== current.objectIdentity.externalId
  ...
  const identityObjects = map.nativeObjects.filter((object) =>
    sameIdentity(object.objectIdentity, current.objectIdentity)).sort((left, right) =>
    Date.parse(left.occurredAt) - Date.parse(right.occurredAt)
      || compare(left.relativePath, right.relativePath));
  const fieldObservations = identityObjects.map((object) => ({
    object,
    field: fieldFor(object, query.fieldPath),
  })).filter((row): row is { object: SourceNativeObject; field: SourceNativeField } =>
    row.field !== null);
  const latest = fieldObservations.at(-1) ?? fail('SOURCE_NATIVE_CURRENT_FIELD_CHRONOLOGY_FIELD');
  const mappedCurrent = latest.object.relativePath === current.relativePath
  ...
  const revisions = map.fieldRevisions.filter((revision) =>
    revision.objectIdentitySha256 === latest.object.objectIdentitySha256
      && revision.fieldPath === query.fieldPath).sort((left, right) =>
  ...
  if (exactResolution.revisionClosureCount !== revisions.length
  ...
  if (map.sourceCount !== sourceHandles.length || map.mappedSourceCount !== map.sourceCount
    || map.unsupportedSourceCount !== 0 || mappedPaths.size !== sourceHandles.length
  ...
  const latestValues = new Set(fieldObservations
  ...
  const core = {
    ...
    state: proofDisposition === 'sufficient'
