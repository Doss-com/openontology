...
function normalizeObligation(value: unknown): ProofSufficiencyObligation {
  ...
  if (
    Object.keys(row).some(
      (key) =>
        ![
          ...
          'relationshipTargetPropositionFamily',
          'required',
          'role',
    ...
    !row.obligationId ||
    typeof row.propositionFamily !== 'string' ||
    !row.propositionFamily ||
    ...
    (row.minimumCount !== undefined &&
      (!Number.isSafeInteger(row.minimumCount) ||
        (row.minimumCount as number) < 0 ||
        (row.minimumCount as number) > 128)) ||
    ...
    (row.sameFamilyAsObligationId !== undefined &&
      (typeof row.sameFamilyAsObligationId !== 'string' || !row.sameFamilyAsObligationId))
  )
    fail();
  ...
  const expectedSourceProjectionItemIds =
    expectedSourceProjectionItemIdValues === undefined
      ? null
      : stringList(expectedSourceProjectionItemIdValues);
  if (
    (expectedSourceProjectionItemIdValues !== undefined &&
      (!Array.isArray(expectedSourceProjectionItemIdValues) ||
        expectedSourceProjectionItemIds === null ||
        expectedSourceProjectionItemIds.length !== expectedSourceProjectionItemIdValues.length)) ||
    (row.minimumCount !== undefined &&
      expectedSourceProjectionItemIds !== null &&
...
export function compileProofSufficiencyContract({
  ...
  const exactQuestionKind =
    typeof questionKind === 'string' && questionKind ? questionKind : fail();
  const exactSufficiencyRule =
    typeof sufficiencyRule === 'string' && sufficiencyRule ? sufficiencyRule : fail();
  ...
  const authority =
    sourceProjectionAuthority === undefined
      ? null
      : normalizeSourceProjectionAuthority(sourceProjectionAuthority);
  if (
    new Set(normalized.map((row) => row.obligationId)).size !== normalized.length ||
    !normalized.some((row) => row.required) ||
    !normalized.some(
      (row) => row.required && row.role === 'support' && row.propositionFamily === 'exact-support',
    ) ||
    normalized.some((row) => row.role === 'invalidator' && !row.required) ||
    normalized.some(
      (row) =>
        row.role === 'invalidator' &&
    ...
    (authority === null && normalized.some((row) => row.role === 'invalidator'))
  )
    fail();
  ...
  if (
    ...
    fail();
  const requiredPropositionFamilies = [
    ...new Set(normalized.filter((row) => row.required).map((row) => row.propositionFamily)),
  ].sort(compare);
  const core = {
