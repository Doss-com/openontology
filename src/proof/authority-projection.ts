...
const exactTimeOrNull = (value: unknown): value is string | null =>
  value === null ||
  (typeof value === 'string' &&
...
const sha256String = (value: unknown): string =>
  typeof value === 'string' && SHA256.test(value) ? value : fail();
const safeInteger = (value: unknown): number =>
  typeof value === 'number' && Number.isSafeInteger(value) ? value : fail();
...
export function normalizeProofAuthorityItem(value: unknown): ProofAuthorityItem {
  ...
  if (
    canonicalRoleValues.length < 1 ||
    !canonicalRoleValues.every((role) => typeof role === 'string' && role.length > 0)
  )
    fail();
  ...
  if (
    new Set(exactEvidenceReferences.map(stableObjectText)).size !== exactEvidenceReferences.length
  )
    fail();
  return freeze({
    sourceProjectionItemId,
...
export function normalizeProofAuthorityRelation(
  value: unknown,
  itemIds: Set<string>,
  ...
  if (
    sourceProjectionItemId === targetProjectionItemId ||
    !itemIds.has(sourceProjectionItemId) ||
    !itemIds.has(targetProjectionItemId)
  )
    fail();
  return freeze({
    type,
...
export function normalizeSourceProjectionAuthority(value: unknown): SourceProjectionAuthority {
  const row = plain(value) ? value : fail();
  if (!exactKeys(row, ['sourceProjectionKind', 'sourceProjectionSha256', 'proofCensusSha256']))
    fail();
  return freeze({
...
export function compileProofAuthorityProjection({
  ...
  const items = unknownList(inputItems)
    .map(normalizeProofAuthorityItem)
  ...
  if (
    relations.some(
      (relation) =>
        COUNTEREVIDENCE_RELATION_TYPES.has(relation.type) &&
        !itemById.get(relation.sourceProjectionItemId)?.canonicalRoles.includes('counterevidence'),
    )
  )
    fail();
  const core = {
