...
import { objectBytesSha256, stableObjectSha256 } from '../canonical-content.js';
import { openSourceNativeProductWithAdmittedKnowledge } from '../ledger/admitted-knowledge.js';
import type { SourceNativeAdmissionTrustEntry } from '../ledger/admitted-knowledge.js';
...
import type { SourceNativeSemanticWitness } from './compiler.js';
...
export type SourceNativeConstructionLedgerSummary = Omit<
  SourceNativeConstructionLedger,
  'activeRecords'
> & {
...
function searchInput(input: SourceNativeConstructionSearchInput) {
  ...
  const scope =
    value.scope === undefined
      ...
      : (() => {
          ...
          return {
            sourceSystem: text(scopeInput.sourceSystem),
            ...(scopeInput.objectType === undefined
              ? {}
              : { objectType: text(scopeInput.objectType) }),
          };
        })();
  ...
  if (typeof limit !== 'number' || !Number.isSafeInteger(limit) || limit < 1 || limit > 64)
    fail('INPUT');
...
export function openSourceNativeProductWithConstruction(
  ...
  const reader = createConstructionLedgerReader(
    context,
    configuration.trustRegistry,
    configuration.knowledgeBranch,
  );
  let ledger = reader.read();
  const objects = new Map(
    context.objectOnt.map.nativeObjects.map((object) => [object.nativeObjectSha256, object]),
  );
  ...
  const search = async (input: ProductSearchInput | SourceNativeConstructionSearchInput) => {
    ...
    if (page && (page.querySha256 !== querySha256 || page.projectionSha256 !== projectionSha256))
      fail('CURSOR_STALE');
    const offset = page?.offset ?? 0;
    const constructions = new Map<string, SourceNativeConstructionAdmissionRecord>();
    for (const record of [...ledger.activeRecords].sort((a, b) =>
      compare(a.recordSha256, b.recordSha256),
    )) {
      if (!constructions.has(record.construction.constructionSha256))
        constructions.set(record.construction.constructionSha256, record);
    }
    ...
    for (const record of constructions.values()) {
      for (const definition of record.construction.objectDefs) {
        ...
        if (
          normalized(definition.name) !== normalized(term) &&
          !definition.aliases.some(
            (alias) =>
              normalized(alias.value) === normalized(term) &&
              (scope === null || alias.sourceSystem === scope.sourceSystem),
          )
        )
          continue;
        ...
        const add = (
          source: SourceNativeSemanticWitness,
          role: SourceNativeConstructionAttachmentRole,
        ) => {
          const object = objects.get(source.nativeObjectSha256) ?? fail('BINDING');
          if (
            scope &&
            (object.objectIdentity.sourceSystem !== scope.sourceSystem ||
              (scope.objectType !== undefined &&
                object.objectIdentity.objectType !== scope.objectType))
          )
            return;
        ...
        for (const claim of record.construction.claims.filter(
          (claim) => claim.about === definition.id,
        ))
          add(claim.source, claim.predicate);
        if (passages.size > 0)
          concepts.push({
            id: definition.id,
            ...
            passages: [...passages]
              ...
              .map(([, passage]) => ({ ...passage, roles: passage.roles.sort(compare) })),
          });
      }
    }
    ...
    const state: SourceNativeConstructionSearchResult['state'] =
      concepts.length > 1
        ? 'ambiguous-construction-navigation'
        : concepts.length === 1
          ? 'resolved-construction-navigation'
    ...
    const matches = selected.map((passage): SourceNativeConstructionMatch => {
      ...
      return {
        ...
        nativeObject: passage.object.objectIdentity,
    ...
    const pageConcepts = concepts
      .filter((concept) => visibleConceptIds.has(concept.id))
      .map(({ id, name }) => ({ id, name }));
    const core = {
      schemaVersion: 1 as const,
      kind: 'OpenOntologyConstructionSearchResultV1' as const,
      state,
      query: { term, scope, conceptId },
    ...
    return freeze({
      ...core,
      resultSha256: stableObjectSha256(core),
    }) satisfies SourceNativeConstructionSearchResult;
  };
  ...
  const read = async (input: { ref: string }) => {
    ...
    ledger = reader.read();
    if (!ledger.activeRecords.some((record) => record.recordSha256 === passage.record.recordSha256))
      fail('REFERENCE_INELIGIBLE');
    ...
    if (
      object.objectIdentity.namespace !== context.descriptor.namespace ||
      object.relativePath !== reference.sourceRef ||
      object.sourceSha256 !== reference.sourceSha256 ||
      ...
      !object.fields.some(
        (field) =>
          field.evidence.relativePath === reference.sourceRef &&
          field.evidence.sourceSha256 === reference.sourceSha256 &&
          field.evidence.byteStart <= reference.byteStart &&
          field.evidence.byteEnd >= reference.byteEnd,
      )
    )
      fail('EVIDENCE');
    const core = {
      schemaVersion: 1 as const,
      kind: 'OpenOntologyConstructionReadResultV1' as const,
      ref,
      exactText,
      evidence: reference,
      binding: {
        kind: 'OpenOntologyConstructionPassageBindingV1' as const,
        conceptId: passage.conceptId,
        roles: passage.roles,
        ...
        admissionRecordSha256: passage.record.recordSha256,
        navigationOnly: true as const,
        exactSourcesRemainAuthority: true as const,
      },
      sourceCommitSha256: context.objectOnt.commitSha256,
    ...
    }) satisfies SourceNativeConstructionReadResult;
  };
  return freeze({
    ...base,
    kind: 'OpenOntologySourceNativeConstructionProductV1' as const,
    search,
    read,
    status: () => freeze({ ...base.status(), construction: summary(ledger) }),
  });
}

export type SourceNativeConstructionProduct = ReturnType<
  typeof openSourceNativeProductWithConstruction
>;
