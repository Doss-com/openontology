...
} from '../ledger/authentication.js';
import type { SourceNativeAdmissionTrustEntry } from '../ledger/authentication.js';
import { openExactProductArtifactState, openProductSourceContext } from '../source/artifact.js';
import type { ProductOptions, SourceNativeProductSourceContext } from '../source/artifact.js';
import type {
  BlobDescriptor,
  ReplayMetadataGraph,
...
const KNOWN_READER_ERROR_CODES = new Set([
  'CONSTRUCTION_ADMISSION_SHAPE',
  'CONSTRUCTION_ADMISSION_TEXT',
  'CONSTRUCTION_ADMISSION_HASH',
  'CONSTRUCTION_ADMISSION_BRANCH',
  'CONSTRUCTION_ADMISSION_EMPTY',
  'CONSTRUCTION_ADMISSION_TIME',
  'CONSTRUCTION_ADMISSION_INDEPENDENCE',
  'CONSTRUCTION_ADMISSION_SUPERSESSION',
  'CONSTRUCTION_ADMISSION_STATEMENT',
  'CONSTRUCTION_ADMISSION_LIMIT',
  'CONSTRUCTION_ADMISSION_RECORD',
  'CONSTRUCTION_ADMISSION_PATH',
  'CONSTRUCTION_ADMISSION_MISSING',
  'CONSTRUCTION_ADMISSION_ROLLBACK',
  'CONSTRUCTION_ADMISSION_AMBIGUOUS',
  'SOURCE_NATIVE_ADMISSION_SIGNATURE',
  'SOURCE_NATIVE_ADMISSION_TRUST',
  'SOURCE_NATIVE_ADMISSION_AUTHENTICATION',
  'SEMANTIC_CONSTRUCTION_SHAPE',
  'SEMANTIC_CONSTRUCTION_TEXT',
  'SEMANTIC_CONSTRUCTION_HASH',
  'SEMANTIC_CONSTRUCTION_ID',
  'SEMANTIC_CONSTRUCTION_COUNT',
  'SEMANTIC_CONSTRUCTION_LIMIT',
  'SEMANTIC_CONSTRUCTION_DUPLICATE',
  'SEMANTIC_CONSTRUCTION_SPAN',
  'SEMANTIC_CONSTRUCTION_TIME',
  'SEMANTIC_CONSTRUCTION_METHOD',
  'SEMANTIC_CONSTRUCTION_KIND',
  'SEMANTIC_CONSTRUCTION_ENDPOINT',
  'SEMANTIC_CONSTRUCTION_PREDICATE',
  'SEMANTIC_CONSTRUCTION_COVERAGE',
  'SEMANTIC_CONSTRUCTION_RECORD',
  'SEMANTIC_CONSTRUCTION_BINDING',
  'SEMANTIC_CONSTRUCTION_SOURCE',
  'SEMANTIC_CONSTRUCTION_EVIDENCE',
  'SEMANTIC_CONSTRUCTION_NAME',
  'SEMANTIC_CONSTRUCTION_SCOPE',
  'SEMANTIC_CONSTRUCTION_ATTACHMENT',
  'OBJECT_ONT_HISTORY_CONFLICT',
  'OBJECT_ONT_HISTORY_CORRUPT',
  'OBJECT_ONT_HISTORY_TARGET',
  'OBJECT_ONT_HISTORY_MISSING',
  'OBJECT_ONT_HISTORY_PENDING',
  'OBJECT_ONT_HISTORY_MISMATCH',
  'OBJECT_ONT_HISTORY_ENROLLMENT_RACE',
  'OBJECT_ONT_HISTORY_BACKEND',
  'OBJECT_ONT_HISTORY_WRITE',
  'OBJECT_ONT_REF_READ',
  'OBJECT_ONT_REF_SCOPE',
  'OBJECT_ONT_REF_ROLLBACK',
  'OBJECT_ONT_REF_CONFLICT',
  'OBJECT_ONT_COMMIT_READ',
  'OBJECT_ONT_BLOB_READ',
  'OBJECT_ONT_REPLAY_INDEX_CHECKPOINT_READ',
...
function descriptorSourceBinding(
  state: ConstructionLedgerContext,
): SourceNativeSemanticSourceBinding {
  return {
...
function readProtectedSnapshot(
  state: ConstructionLedgerContext,
  branch: string,
...
function assertSourceAncestry(
  state: ConstructionLedgerContext,
  snapshot: ReplayMetadataSnapshot | null,
): void {
...
function assertSelectionStable(
  before: ReplayMetadataSnapshot | null,
  after: ReplayMetadataSnapshot | null,
): void {
  if (selectionFingerprint(before) !== selectionFingerprint(after)) fail('CONCURRENT');
}
function recordPath(sha256: string): string {
  return `${PREFIX}${sha256.slice(7)}.json`;
}
function branchFor(state: ConstructionLedgerContext, input: unknown): string {
  const branch =
    input === undefined ? `knowledge-${state.objectOnt.commitSha256.slice(7, 23)}` : text(input);
...
export function sourceNativeConstructionProposalStatement({
  construction: input,
}: {
  ...
  return freeze({
    ...
    proposerId: construction.proposedBy,
    proposedAt: construction.proposedAt,
    signatureAlgorithm: 'Ed25519',
    navigationOnly: true,
    exactSourcesRemainAuthority: true,
  });
...
export function sourceNativeConstructionAdmissionStatement({
  ...
  if (!Number.isFinite(Date.parse(admittedAt)) || new Date(admittedAt).toISOString() !== admittedAt)
    fail('TIME');
  if (
    issuerId === construction.proposedBy ||
    Date.parse(admittedAt) < Date.parse(construction.proposedAt)
  )
    fail('INDEPENDENCE');
  ...
  return freeze({
    ...
    kind: 'OpenOntologySourceNativeConstructionAdmissionStatementV1',
    constructionSha256: construction.constructionSha256,
    proposedBy: construction.proposedBy,
    issuerId,
    admittedAt,
...
export function compileSourceNativeConstructionAdmissionRecord({
  construction: input,
  proposalStatement: proposerInput,
  proposalSignatureBase64,
  statement: reviewerInput,
  signatureBase64,
}: {
  construction: unknown;
  ...
  const reviewer = row(reviewerInput, [
    'schemaVersion',
    'kind',
    'constructionSha256',
    'proposedBy',
  ...
  const statement = sourceNativeConstructionAdmissionStatement({
    construction,
    issuerId: text(reviewer.issuerId),
    admittedAt: text(reviewer.admittedAt),
  ...
  const core = {
    ...
    kind: 'OpenOntologySourceNativeConstructionAdmissionRecordV1' as const,
    construction,
    proposalStatement,
    proposalSignatureBase64: canonicalAdmissionSignature(proposalSignatureBase64),
    statement,
    signatureBase64: canonicalAdmissionSignature(signatureBase64),
  };
...
export function validateSourceNativeConstructionAdmissionRecord(
  value: unknown,
): SourceNativeConstructionAdmissionRecord {
  ...
  const expected = compileSourceNativeConstructionAdmissionRecord({
    construction: input.construction,
    proposalStatement: input.proposalStatement,
    proposalSignatureBase64: input.proposalSignatureBase64,
...
function assertReplay(state: ConstructionLedgerContext, replay: ReplayMetadataGraph): void {
  if (
    replay.status !== 'CLEAN' ||
    replay.ontId !== state.descriptor.ontId ||
    !replay.commitOrder.includes(state.objectOnt.commitSha256)
  )
    fail('BRANCH');
}
function assertCorrection(
  record: SourceNativeConstructionAdmissionRecord,
  target: SourceNativeConstructionAdmissionRecord,
): void {
  if (
    Date.parse(target.statement.admittedAt) >= Date.parse(record.statement.admittedAt) ||
    stableObjectText(target.construction.sourceBinding) !==
      stableObjectText(record.construction.sourceBinding) ||
    stableObjectText(target.construction.objectDefs.map((item) => item.id)) !==
      stableObjectText(record.construction.objectDefs.map((item) => item.id))
  )
    fail('SUPERSESSION');
}
...
export function writeSourceNativeConstructionAdmission({
  ...
  for (const targetSha of record.statement.supersedesRecordSha256s) {
    const descriptor = replay?.blobDescriptors.find(
      (item) => item.logicalPath === recordPath(targetSha),
    );
  ...
  const blob = state.store.putBlob({
    logicalPath,
    bytes,
    mediaType: 'application/vnd.openontology.construction-admission+json',
  });
  ...
  return freeze({
    ...
    recordSha256: record.recordSha256,
    commitSha256: receipt.commitSha256,
    replaySha256: updated.ref.replaySha256,
    replayed: false,
  });
}
...
export function createConstructionLedgerReader(
  ...
  function evaluate(
    ...
    | SourceNativeConstructionLedgerSnapshot
    | SourceNativeConstructionSelectedSnapshot {
    ...
    const dispositions = collectRecords
      ? new Map<string, MutableSourceNativeConstructionRecordDisposition>()
      : null;
    ...
    const bindingContext: ConstructionLedgerContext =
      context.readSource === undefined
        ? context
        : {
            ...context,
    ...
    function invalid(
      error: unknown,
      record: MutableSourceNativeConstructionRecordDisposition | undefined = undefined,
      state: SourceNativeConstructionRecordDispositionState = 'invalid',
      fallback = GENERIC_RECORD_ERROR_CODE,
    ): void {
    ...
    try {
      ...
      if (snapshot && replay) {
        ...
        for (const descriptor of replay.blobDescriptors.filter((item) =>
          item.logicalPath.startsWith(PREFIX),
        )) {
          const disposition: MutableSourceNativeConstructionRecordDisposition | undefined =
            collectRecords
              ? {
          ...
          try {
            ...
            if (disposition)
              dispositions?.set(malformedDispositionKey(disposition.blobSha256), disposition);
            continue;
          }
          if (disposition) {
    ...
    const superseded = new Set(
      eligible.flatMap((record) => record.statement.supersedesRecordSha256s),
    );
    ...
    const recordsByObjectDefId = collectRecords
      ? new Map<string, SourceNativeConstructionAdmissionRecord[]>()
      : null;
    ...
    const conflictingObjectDefIds = [...proposalsById]
      .filter(([, hashes]) => hashes.size > 1)
      .map(([id]) => id)
      .sort(compare);
    const conflicts = new Set(conflictingObjectDefIds);
    const activeRecords = current
      .filter((record) => !record.construction.objectDefs.some((item) => conflicts.has(item.id)))
    ...
    if (collectRecords && recordsByObjectDefId && dispositions && supersededBy) {
      ...
      for (const objectDefId of conflictingObjectDefIds) {
        ...
        for (const record of records) {
          const objectDefIds =
            conflictingObjectIdsByRecord.get(record.recordSha256) ?? new Set<string>();
      ...
      for (const disposition of dispositions.values()) {
        ...
        const conflictObjectIds = [
          ...(conflictingObjectIdsByRecord.get(disposition.recordSha256) ?? []),
        ].sort(compare);
    ...
    const ledger: SourceNativeConstructionLedger = freeze({
      schemaVersion: 1 as const,
      kind: 'OpenOntologySourceNativeConstructionLedgerV1' as const,
      branch,
      commitSha256,
      replaySha256,
      structuralRecordCount: structural.size,
      ...
      activeRecords,
      diagnosticCodes: [...diagnostics].sort(compare),
      state: diagnostics.size > 0 ? 'degraded' : 'ready',
    ...
    if (collectRecords && dispositions && supersededBy && conflictingObjectIdsByRecord) {
      const recordStates = [...dispositions.values()]
        .map((record): SourceNativeConstructionRecordDisposition => {
          ...
          const superseders =
            recordSha256 === null ? [] : [...(supersededBy.get(recordSha256) ?? [])].sort(compare);
          const conflictObjectIds =
            recordSha256 === null
              ? []
          ...
          return {
            ...
            supersedesRecordSha256s:
              recordSha256 === null ? [] : [...record.supersedesRecordSha256s].sort(compare),
      ...
      if (selected) {
        const selectedDisposition =
          recordStates.find((item) => item.recordSha256 === selected.recordSha256) ??
          (() => {
            const descriptor = selected.replay.blobDescriptors.find(
              (item) => item.logicalPath === recordPath(selected.recordSha256),
            );
        ...
        return freeze({
          ...
          selectedRecord,
          selectedDisposition,
          knowledgeSnapshot: selected.knowledgeSnapshot,
  ...
  const readSnapshotAt = (input: {
    ...
    if (
      !SHA256.test(input.commitSha256) ||
      !SHA256.test(input.replaySha256) ||
      !SHA256.test(input.recordSha256)
    )
      fail('HASH');
    ...
    if (
      replay.status !== 'CLEAN' ||
      replay.ontId !== context.descriptor.ontId ||
      replay.replaySha256 !== input.replaySha256
    )
      fail('BRANCH');
    assertReplay(context, replay);
    ...
    if (!sourceSnapshot.replayMetadata.commitOrder.includes(context.objectOnt.commitSha256))
      fail('BRANCH');
    const value = evaluate(true, {
      replay,
      commitSha256: input.commitSha256,
...
export function readSourceNativeConstructionLedgerAtArtifact({
  ...
  if (
    state.descriptor.schemaVersion !== 2 ||
    typeof state.descriptor.historyBackend !== 'string' ||
    state.descriptor.historyBackend.length === 0
  )
    fail('HISTORY');
  if (stableObjectText(expected) !== stableObjectText(descriptorSourceBinding(context)))
    fail('BINDING');
