...
async function run(outputRoot) {
  ...
  const sources = [
    {
      relativePath: 'docs/allocation.txt',
      sourceType: 'docs',
      content:
        'AllocationException means inventory allocation mismatch. ClickUp calls it allocation mismatch.',
    },
    {
  ...
  buildSourceNativeProduct({
    ...options,
    historyBackendUri: pathToFileURL(join(outputRoot, 'history')).href,
    input: {
      schemaVersion: 1,
      kind: 'OpenOntologySourceNativeBuildInputV1',
      ontId: 'semantic-map-example',
      namespace: 'example',
      sources,
      querySchemas: [
        {
      ...
      nativeObjectInputs: sources.map((source, index) => ({
        ...
        objectIdentity: {
          ...
          externalId: index ? `CT-${16 + index}` : 'allocation-guide',
        },
        fields: [
  ...
  const witness = (relativePath) => {
    const object = state.objectOnt.map.nativeObjects.find(
      (object) => object.relativePath === relativePath,
    );
  ...
  const input = {
    ...
    objectDefs: [
      {
        ...
        source: witness(sources[0].relativePath),
        aliases: [
          {
    ...
    claims: sources.map((source, index) => ({
      ...
      predicate: index ? 'mentions' : 'defines',
  ...
  const trustRegistry = [
    ['constructor', constructor, 'proposer'],
    ['reviewer', reviewer, 'reviewer'],
  ].map(([issuerId, pair, role]) => ({
    issuerId,
    roles: [role],
    publicKeyPem: pair.publicKey.export({ type: 'spki', format: 'pem' }),
  }));
  const signature = (statement, pair) =>
    sign(null, Buffer.from(stableObjectText(statement)), pair.privateKey).toString('base64');
  function admit(input, supersedesRecordSha256s = []) {
    ...
    const statement = sourceNativeConstructionAdmissionStatement({
      construction,
      issuerId: 'reviewer',
      admittedAt: supersedesRecordSha256s.length
        ? '2026-09-04T00:00:00.000Z'
        : '2026-09-03T00:00:00.000Z',
      supersedesRecordSha256s,
    });
    const record = compileSourceNativeConstructionAdmissionRecord({
      construction,
      proposalStatement,
      statement,
      proposalSignatureBase64: signature(proposalStatement, constructor),
  ...
  const second = await ont.search({
    term: 'allocation mismatch',
    scope: { sourceSystem: 'clickup', objectType: 'ClickupTask' },
    cursor: search.nextCursor,
  });
  const selected = [...search.matches, ...second.matches].find(
    (match) => match.nativeObject.externalId === 'CT-17',
  );
  const passage = await ont.read({ ref: selected.ref });
  assert(passage.exactText.includes('allocation mismatch'));
  const query = {
    question: 'What is the current status?',
    typedQuery: { sourceSystem: 'clickup', objectType: 'ClickupTask', fieldPath: 'status' },
  };
  assert.equal((await ont.verify(query)).answerable, false);
  const verified = await ont.verify({
    ...query,
    typedQuery: { ...query.typedQuery, externalId: passage.binding.nativeObject.externalId },
  });
  assert.equal(verified.context[0].exactText, 'open');
  const correction = admit(
    { ...input, claims: input.claims.filter((claim) => claim.id !== 'attachment-2') },
    [original.recordSha256],
  );
  await assert.rejects(ont.read({ ref: selected.ref }), {
    code: 'CONSTRUCTION_NAVIGATION_REFERENCE_INELIGIBLE',
  });
  const after = await ont.search({
    term: 'allocation mismatch',
  ...
  assert.equal(
    (await cold.search({ term: 'allocation mismatch', scope: { sourceSystem: 'clickup' } }))
      .totalMatches,
    1,
  );
  const revoked = openSourceNativeProductWithConstruction(options, {
    trustRegistry: trustRegistry.slice(0, 1),
  });
  assert.equal((await revoked.search({ term: 'AllocationException' })).matches.length, 0);
  process.stdout.write(
    `${JSON.stringify({
      kind: 'OpenOntologySemanticMapWalkthroughV1',
      outputRoot,
      initialMatches: search.totalMatches,
      correctedMatches: after.totalMatches,
      selectedObject: passage.binding.nativeObject.externalId,
      exactStatus: verified.context[0].exactText,
      correctionRecordSha256: correction.recordSha256,
      semanticReviewQualified: false,
    })}\n`,
  );
}
