...
function faultBackend(raw, { failCasAt = [], keyPrefix = null } = {}) {
  ...
  return {
    ...
    compareAndSwap(key, options) {
      calls += 1;
      if ((!keyPrefix || key.startsWith(keyPrefix)) && failCasAt.includes(calls))
        backendError('OBJECT_BACKEND_PRECONDITION');
...
function readBoundaryBackend(
  ...
  return {
    ...
    get(key, options) {
      ...
      if (corrupt === 'bytes')
        return { ...result, bytes: Buffer.from(`${result.bytes.toString('utf8')} `) };
      return result;
    },
    putIfAbsent: (...args) => raw.putIfAbsent(...args),
...
test('cold protected readers refuse a rewind and recover only the witnessed target', () => {
  ...
  assert.equal(
    cold.readRefMetadata({ ontId: 'history-fixture', branch: 'main' }).ref.commitSha256,
    second.commitSha256,
  );
});
...
test('enrollment is explicit, legacy mode remains available, and historical reads stay immutable', () => {
  ...
  assert.equal(
    protectedStore.readRefHead({ ontId: 'history-enrollment', branch: 'main' }).ref.commitSha256,
    commit.commitSha256,
  );
  ...
  assert.deepEqual(
    protectedStore.readCommit(commit.commitSha256).commit,
    legacy.readCommit(commit.commitSha256).commit,
  );
  assert.equal(
    protectedStore.replayMetadata(commit.commitSha256).tipCommitSha256,
    commit.commitSha256,
  );
});
...
test('protected publication covers ordinary, metadata, and checkpointed variants', () => {
  ...
  for (const [name, publish] of variants) {
    ...
    assert.equal(
      store.readRefHead({ ontId: `history-variant-${name}`, branch: 'main' }).ref.commitSha256,
      second.commitSha256,
    );
    assert.equal(
      store.readRef({ ontId: `history-variant-${name}`, branch: 'main' }).ref.commitSha256,
      second.commitSha256,
    );
    assert.equal(
      store.readRefMetadata({ ontId: `history-variant-${name}`, branch: 'main' }).ref.commitSha256,
      second.commitSha256,
    );
    assert.equal(
      store.readRefMetadataSnapshot({ ontId: `history-variant-${name}`, branch: 'main' }).ref
        .commitSha256,
      second.commitSha256,
    );
    assert.equal(
      store.readRefMetadataCheckpointSnapshot({ ontId: `history-variant-${name}`, branch: 'main' })
        .ref.commitSha256,
      second.commitSha256,
    );
...
test('protected recovery refuses an unaccepted third state and malformed history', () => {
  ...
  data.overwrite(
    'refs/history-negative/main.json',
    Buffer.from(
      stableObjectText({
        ...firstResult.ref,
...
test('protected data and history observations distinguish drift from same-version corruption', () => {
  ...
  writer.compareAndSwapRefMetadata({
    ontId: 'history-observation',
    branch: 'main',
    commitSha256: commit.commitSha256,
  });

  const driftedData = openObjectOntStore({
    backend: readBoundaryBackend(data, { mutateOnGet: true }),
  ...
  for (const corrupt of ['key', 'checksum', 'length', 'bytes']) {
    const corruptedData = openObjectOntStore({
      backend: readBoundaryBackend(data, { corrupt }),
      historyBackend: history,
    });
    assert.throws(
      () => corruptedData.readRefHead({ ontId: 'history-observation', branch: 'main' }),
      {
        code: 'OBJECT_ONT_HISTORY_CORRUPT',
      },
    );
  ...
  assert.throws(
    () => corruptedHistory.readRefHead({ ontId: 'history-observation', branch: 'main' }),
    {
      code: 'OBJECT_ONT_HISTORY_CORRUPT',
    },
  );

  for (const throwCode of [
    'OBJECT_BACKEND_NOT_FOUND',
    'OBJECT_BACKEND_CORRUPT',
    'OBJECT_BACKEND_TIMEOUT',
  ]) {
    const thrownData = openObjectOntStore({
      backend: readBoundaryBackend(data, { throwCode }),
      historyBackend: history,
    });
    assert.throws(() => thrownData.readRefHead({ ontId: 'history-observation', branch: 'main' }), {
      code:
        throwCode === 'OBJECT_BACKEND_NOT_FOUND'
          ? 'OBJECT_ONT_HISTORY_CONFLICT'
          : throwCode === 'OBJECT_BACKEND_CORRUPT'
            ? 'OBJECT_ONT_HISTORY_CORRUPT'
            : throwCode,
    });
    const thrownHistory = openObjectOntStore({
      backend: data,
      historyBackend: readBoundaryBackend(history, { throwCode }),
    });
    assert.throws(
      () => thrownHistory.readRefHead({ ontId: 'history-observation', branch: 'main' }),
      {
        code:
          throwCode === 'OBJECT_BACKEND_NOT_FOUND'
            ? 'OBJECT_ONT_HISTORY_CONFLICT'
  ...
  assert.throws(
    () => disappearedData.readRefHead({ ontId: 'history-observation', branch: 'main' }),
    {
      code: 'OBJECT_ONT_HISTORY_CONFLICT',
    },
  );
  ...
  assert.throws(
    () => disappearedHistory.readRefHead({ ontId: 'history-observation', branch: 'main' }),
    {
      code: 'OBJECT_ONT_HISTORY_CONFLICT',
    },
  );
...
test('accepted-head deletion is repaired only to the protected target', () => {
  ...
  assert.equal(
    store.readRefHead({ ontId: 'history-deleted-head', branch: 'main' }).ref.commitSha256,
    commit.commitSha256,
  );
});
...
test('enrollment detects a ref advance during the history CAS', () => {
  ...
  assert.throws(
    () =>
      openObjectOntStore({ backend: data, historyBackend: history }).readRefHead({
        ontId: 'history-enrollment-race',
        branch: 'main',
      }),
    {
...
test('reservation, data-ref, and finalization failures leave recoverable exact targets', () => {
  ...
  for (const [name, failingSide, failCasAt] of variants) {
    ...
    const history = openMemoryObjectBackend();
    const dataBackend =
      failingSide === 'data' ? faultBackend(data, { failCasAt, keyPrefix: 'refs/' }) : data;
    const historyBackend =
      failingSide === 'history'
        ? faultBackend(history, { failCasAt, keyPrefix: 'ref-history/' })
        : history;
    ...
    if (name === 'reservation') {
      ...
      assert.throws(
        () => recovering.readRefHead({ ontId: `history-fault-${name}`, branch: 'main' }),
        {
      ...
      assert.equal(
        recovering.readRefHead({ ontId: `history-fault-${name}`, branch: 'main' }).ref.commitSha256,
        second.commitSha256,
      );
    }
  }
  ...
  assert.throws(
    () =>
      store.compareAndSwapRefMetadata({
        ontId: 'history-publish-payload',
        branch: 'main',
        commitSha256: target.commitSha256,
      }),
    { code: 'OBJECT_ONT_HISTORY_TARGET' },
  );
  assert.throws(
...
test('recovery refuses a target when an older historical source manifest is unavailable', () => {
  ...
  const firstManifest = store.putBlob({
    logicalPath: 'oont.json',
    bytes: Buffer.from('{"cut":1}'),
    mediaType: 'application/json',
  });
  const secondManifest = store.putBlob({
    logicalPath: 'oont.json',
    bytes: Buffer.from('{"cut":2}'),
    mediaType: 'application/json',
  });
  const first = store.writeCommitMetadata({
    ontId: 'history-manifest',
    ontManifest: firstManifest,
  });
  const second = store.writeCommitMetadata({
    ontId: 'history-manifest',
    parents: [first.commitSha256],
    ontManifest: secondManifest,
  });
  const firstResult = store.compareAndSwapRefMetadata({
    ontId: 'history-manifest',
    branch: 'main',
...
test('target validation preserves provider transport failures at commit and ancestor reads', () => {
  ...
  const targetRead = openObjectOntStore({
    backend: readBoundaryBackend(data, {
      throwCode: 'OBJECT_BACKEND_TIMEOUT',
      throwOnKey: second.key,
    }),
    historyBackend: history,
  });
  assert.throws(
    () =>
      targetRead.compareAndSwapRefMetadata({
        ontId: 'history-target-transport',
        branch: 'main',
        commitSha256: second.commitSha256,
      }),
  ...
  const ancestorRead = openObjectOntStore({
    backend: readBoundaryBackend(data, {
      throwCode: 'OBJECT_BACKEND_TIMEOUT',
      throwOnKey: first.key,
    }),
    historyBackend: history,
  });
  assert.throws(
    () =>
      ancestorRead.recoverRefHistory({
        ontId: 'history-target-transport',
        branch: 'main',
      }),
    { code: 'OBJECT_BACKEND_TIMEOUT' },
...
test('mutate-then-throw CAS boundaries recover without aborting the reservation', () => {
  {
    ...
    assert.throws(
      () =>
        store.compareAndSwapRefMetadata({
          ontId: 'history-uncertain-reservation',
          branch: 'main',
          commitSha256: commit.commitSha256,
        }),
    ...
    assert.equal(
      recovery.readRefHead({ ontId: 'history-uncertain-reservation', branch: 'main' }).ref
        .commitSha256,
      commit.commitSha256,
    );
    assert.throws(
...

  {
    ...
    const history = openMemoryObjectBackend();
    const store = openObjectOntStore({
      backend: mutateThenThrowBackend(data, { casAt: [1], keyPrefix: 'refs/' }),
      historyBackend: history,
    });
    const manifest = manifestFor(store);
    ...
    assert.equal(
      recovery.readRefHead({ ontId: 'history-uncertain-data', branch: 'main' }).ref.commitSha256,
      commit.commitSha256,
    );
    assert.throws(
      () =>
...

  {
    ...
    assert.throws(
      () =>
        store.compareAndSwapRefMetadata({
          ontId: 'history-uncertain-finalization',
          branch: 'main',
          commitSha256: commit.commitSha256,
        }),
      { code: 'OBJECT_BACKEND_UNCERTAIN' },
    ...
    const recovery = openObjectOntStore({ backend: data, historyBackend: history });
    assert.equal(
      recovery.readRefHead({ ontId: 'history-uncertain-finalization', branch: 'main' }).ref
        .commitSha256,
      commit.commitSha256,
    );
  ...
  const pendingFixture = (label) => {
    ...
    const failedData = openObjectOntStore({
      backend: faultBackend(data, { failCasAt: [1], keyPrefix: 'refs/' }),
      historyBackend: history,
    });
    assert.throws(
      () =>
        failedData.compareAndSwapRefMetadata({
          ontId: label,
          branch: 'main',
          expectedVersion: firstResult.version,
          commitSha256: second.commitSha256,
...

  {
    ...
    assert.throws(
      () =>
        recovering.recoverRefHistory({ ontId: 'history-uncertain-recovery-data', branch: 'main' }),
      {
        code: 'OBJECT_BACKEND_UNCERTAIN',
      },
    );
    ...
    assert.equal(
      clean.readRefHead({ ontId: 'history-uncertain-recovery-data', branch: 'main' }).ref
        .commitSha256,
...

  {
    ...
    const cleanStore = openObjectOntStore({
    ...
    const recovering = openObjectOntStore({
      backend: fixture.data,
      historyBackend: mutateThenThrowBackend(fixture.history, {
        casAt: [1],
        keyPrefix: 'ref-history/',
      }),
    });
    assert.throws(
      () =>
        recovering.recoverRefHistory({
          ontId: 'history-uncertain-recovery-finalization',
          branch: 'main',
        }),
      { code: 'OBJECT_BACKEND_UNCERTAIN' },
    );
    const clean = openObjectOntStore({ backend: fixture.data, historyBackend: fixture.history });
    assert.equal(
      clean.readRefHead({ ontId: 'history-uncertain-recovery-finalization', branch: 'main' }).ref
        .commitSha256,
      fixture.second.commitSha256,
    );
  }
...
test('a fresh third writer cannot advance while recovery finalizes a pending target', () => {
  ...
  const firstResult = base.compareAndSwapRefMetadata({
    ontId: 'history-concurrent-recovery',
    branch: 'main',
    commitSha256: first.commitSha256,
  });
  ...
  assert.throws(
    () =>
      failed.compareAndSwapRefMetadata({
        ontId: 'history-concurrent-recovery',
        branch: 'main',
        expectedVersion: firstResult.version,
        commitSha256: second.commitSha256,
      ...
      if (key === 'ref-history/history-concurrent-recovery/main.json') {
        try {
          originalWriter.compareAndSwapRefMetadata({
            ...
            branch: 'main',
            expectedVersion: firstResult.version,
            commitSha256: third.commitSha256,
          });
        } catch (error) {
          writerError = error;
        }
  ...
  assert.equal(
    recovery.readRefHead({ ontId: 'history-concurrent-recovery', branch: 'main' }).ref.commitSha256,
    second.commitSha256,
  );
});
...
test('the original reservation resumes after reentrant recovery and converges or conflicts', () => {
  ...
  try {
    originalResult = original.compareAndSwapRefMetadata({
      ontId,
      branch: 'main',
      commitSha256: target.commitSha256,
    });
  ...
  const cold = openObjectOntStore({ backend: data, historyBackend: history });
  assert.equal(
    cold.readRefMetadata({ ontId, branch: 'main' }).ref.commitSha256,
    target.commitSha256,
  );
