...
function sourceVersionFixture() {
  ...
  const build = (name, selectedInput = input, version = undefined) =>
    buildSourceNativeProduct({
  ...
  try {
    ...
    assert.throws(
      () =>
        build(
          'delayed',
          successor('Delayed obsolete input', '2026-02-15T00:00:00.000Z'),
          first.receipt.refVersion,
        ),
      {
    ...
    const result = await openOntology({ artifactRoot: join(root, 'newer') }).verify(
...
test('source-version publication keeps initial creation and identical-content retries idempotent', () => {
  ...
  try {
    ...
    const ordinary = build(
      'ordinary',
      successor('Manual observed snapshot', '2026-04-01T00:00:00.000Z'),
    );
...
test('invalid source-version inputs refuse before an output directory is created', () => {
  ...
  try {
    for (const [index, value] of ['', 1, false, {}, []].entries()) {
      ...
      assert.throws(() => build(name, undefined, value), {
        code: 'SOURCE_NATIVE_PRODUCT_SOURCE_VERSION',
      });
...
test('kernel resource binding uses a validated checkpoint without changing verification', async () => {
  ...
  try {
    ...
    const index = openSourceNativeObjectOntRefIndex({
    ...
    assert.equal(
      index.objectOnt.sources.every((source) => source.content === undefined),
      true,
    );
    ...
    const result = await openOntology({ artifactRoot: bound }).verify(
      'What is the current title of task-1?',
    );
    ...
    const poisoned = {
      ...
      get(key, ...args) {
        ...
        return key.startsWith('replay-indexes/')
          ? { ...result, bytes: Buffer.from('poison') }
          : result;
      },
    };
    assert.throws(
