...
function obligation({ id, family, role = 'support', required = true, ...rest }) {
  return {
    obligationId: id,
...
function contract(obligations, { authorityProjection = null } = {}) {
  const authorityObligations =
    authorityProjection !== null && !obligations.some((row) => row.role === 'invalidator')
      ? [...obligations, invalidator()]
      : obligations;
  const completeObligations = authorityObligations.some(
    (row) => row.propositionFamily === 'exact-support' && row.required,
  )
...
test('compiles a content-bound projection-aware proof contract', () => {
  ...
  assert.equal(
    result.sourceProjectionAuthority.proofCensusSha256,
...
test('rejects contract shapes that can silently weaken proof', () => {
  ...
  assert.throws(
    () => contract([obligation({ id: 'support', family: 'outcome', minimumCount: 0 })]),
    /PROOF_SUFFICIENCY_CONTRACT/u,
  );
  assert.throws(() => contract([invalidator()]), /PROOF_SUFFICIENCY_CONTRACT/u);
  assert.throws(
    () =>
      contract([invalidator({ expectedSourceProjectionItemIds: ['counter-1', 'counter-1'] })], {
        authorityProjection,
      }),
    /PROOF_SUFFICIENCY_CONTRACT/u,
  );
  assert.throws(
    () => contract([invalidator({ relationshipAnyOf: [] })], { authorityProjection }),
    /PROOF_SUFFICIENCY_CONTRACT/u,
  );
  assert.throws(
    () => contract([invalidator({ relationshipAnyOf: ['mentions'] })], { authorityProjection }),
    /PROOF_SUFFICIENCY_CONTRACT/u,
  );
  assert.throws(
    () => contract([invalidator({ relationshipAnyOf: ['qualifies'] })], { authorityProjection }),
    /PROOF_SUFFICIENCY_CONTRACT/u,
  );
  assert.throws(
    () => contract([invalidator({ allowedModalities: ['observed'] })], { authorityProjection }),
    /PROOF_SUFFICIENCY_CONTRACT/u,
  );
  assert.throws(
    () =>
  ...
  assert.throws(
    () => contract([obligation({ id: 'outcome', family: 'outcome', allowedModalities: [] })]),
    /PROOF_SUFFICIENCY_CONTRACT/u,
  );
  assert.throws(
    () =>
...
test('rejects contract and authority content that no longer matches its identity', () => {
  ...
  assert.throws(
    () =>
      validateProofSufficiencyContract({
        ...original,
        obligations: original.obligations.map((row) =>
          row.obligationId === 'outcome' ? { ...row, description: 'Weaker.' } : row,
        ),
      }),
    /PROOF_SUFFICIENCY_CONTRACT/u,
  );
    ...
    () =>
      projection(
        ...
        [
          {
            type: 'contradicts',
            sourceRevisionId: 'outcome-1',
            targetRevisionId: 'outcome-2',
          },
        ],
      ),
    /PROOF_AUTHORITY_PROJECTION/u,
...
test('closes coherent action, outcome, actor, chronology, and exact support', () => {
  ...
  assert.equal(
    result.obligations.every((row) => row.state === 'closed'),
    true,
  );
...
test('enforces the immutable invalidator census when expected IDs are omitted', () => {
  ...
  const qualifies = {
    type: 'qualifies',
    sourceRevisionId: 'counter-1',
    targetRevisionId: 'outcome-1',
  };
  const authorityProjection = projection([outcome, counter], [qualifies]);
  const proofContract = contract(
    [obligation({ id: 'outcome', family: 'outcome' }), invalidator()],
    { authorityProjection },
  );
  const result = evaluateProofSufficiencyContract({
    contract: proofContract,
...
test('requires the exact authority projection bound by the contract', () => {
  ...
  const proofContract = contract([invalidator()], { authorityProjection });
        ...
        propositions: [
          {
            ...outcome,
            revisionId: 'missing',
            sourceProjectionItemId: 'missing',
          },
          counter,
        ],
        relations: [],
...
test('does not close an invalidator against a proposed negative target when support requires observed positive', () => {
  ...
  const relation = {
    type: 'qualifies',
    sourceRevisionId: 'counter-1',
    targetRevisionId: 'outcome-proposed-negative',
  };
...
test('applies every required target constraint regardless of obligation order', () => {
  ...
  const observedNegative = proposition({
    id: 'outcome-observed-negative',
    roles: ['outcome'],
    polarity: 'negative',
  });
  const proposedPositive = proposition({
    id: 'outcome-proposed-positive',
    roles: ['outcome'],
    modality: 'proposed',
  });
  const counter = proposition({ id: 'counter-1', roles: ['counterevidence'] });
...
test('requires every authoritative relation that disposes counterevidence', () => {
  ...
  assert.equal(
    obligationResult(partial, 'counter').authoritativeSourceProjectionRelations.length,
    2,
  );
...
test('uses the strongest counterevidence disposition across target families', () => {
  ...
  assert.deepEqual(
    result.projectionRelationCensus.matchedSourceProjectionRelations.map((row) => row.type),
