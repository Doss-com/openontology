...
function historicalInput() {
  ...
  return {
    ...
    namespace: 'history',
    querySchemas: [{ sourceSystem: 'tracker', objectType: 'ticket', aliases: ['ticket'],
      fields: [{ fieldPath: 'status', aliases: ['status'] }] }],
    sources: rows.map(([name, occurredAt, value]) => ({
      relativePath: `tracker/${name}.txt`, sourceType: 'tracker', occurredAt,
      content: `Ticket H-1 status: ${value}.`,
    })),
    nativeObjectInputs: rows.map(([name, , value]) => ({
      relativePath: `tracker/${name}.txt`,
      objectIdentity: { home: 'ObjectDef/InstanceRef', sourceSystem: 'tracker',
        objectType: 'ticket', namespace: 'history', externalId: 'H-1' },
...
test('verifies an earlier field from the complete bound source cut', async (t) => {
  ...
  assert.deepEqual(historical.context.map((row) => row.exactText), ['Draft']);
  ...
  assert.deepEqual(current.context.map((row) => row.exactText), ['Ready']);
});
...
test('public historical verification handles exact validity and source-horizon boundaries', async (t) => {
  ...
  assert.deepEqual(first.context.map((row) => row.exactText), ['Draft']);
  const last = await product.verify(query('2026-08-02T00:00:00.000Z'));
  assert.equal(last.answerable, true);
  assert.deepEqual(last.context.map((row) => row.exactText), ['Ready']);
