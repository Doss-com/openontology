...
test('quickstart CLI, SDK, MCP, and lifecycle recipes remain executable', async () => {
  ...
  try {
    ...
    const build = runCli(['resolver', 'build', input, '--out', artifactRoot]);
    ...
    assert.equal(cliResult.context[0].evidence.relativePath, 'tracker/demo/task-1-v2.txt');

    const cliRefusal = runCli(['verify', artifactRoot, 'Who owns task-1?']);
