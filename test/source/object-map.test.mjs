...
import { objectBytesSha256, stableObjectSha256 } from '../../dist/canonical-content.js';
...
} from '../../dist/source/object-map.js';
...
test('rejects missing, malformed, and inherited map headers at the validator boundary', () => {
  ...
  for (const [label, mutate] of mutations) {
    ...
    mutate(candidate);
    assert.throws(
      () => validateSourceNativeObjectMap(candidate),
      {
...
test('compiles exact source-native field revisions and duplicate evidence without making either authoritative', () => {
  ...
  assert.deepEqual(
    map.fieldRevisions.map((row) => row.fieldPath),
    ['status', 'title'],
  );
  assert.ok(
    map.fieldRevisions.every(
      (row) =>
        row.relationType === 'supersedes' &&
  ...
  assert.equal(
    map.duplicateEvidenceClusters[0].interpretation,
  ...
  assert.deepEqual(
    map.businessEntityEvidenceNeighborhoods.map((row) => row.businessEntityKey),
    ['invoice:1626', 'sku:abc'],
  );
  assert.ok(
    map.businessEntityEvidenceNeighborhoods.every(
      (row) =>
        row.namespace === 'acme' &&
        row.memberCount === 2 &&
        row.interpretation === 'shared-business-entity-key-navigation-not-semantic-equivalence',
    ),
  );
});
...
test('uses a canonical field value to suppress presentation-only revisions while preserving exact Evidence', () => {
  ...
  const map = compileSourceNativeObjectMap({
    ...
    nativeObjectInputs: [
      ...
      },
      {
        ...
        fields: [
          { ...span(newTask, 'assignees', 'Amanda Wang <amanda@example.com>'), canonicalValue },
        ],
      },
    ],
  });
  ...
  assert.deepEqual(map.nativeObjects.map((row) => row.fields[0].value).sort(), [
    'Amanda Wang',
    'Amanda Wang <amanda@example.com>',
  ]);
  assert.ok(
    map.nativeObjects.every((row) => row.fields[0].canonicalValue.home === 'ObjectDef/InstanceRef'),
  );
});
...
test('scopes duplicate Evidence and business-entity neighborhoods by namespace', () => {
  ...
  assert.deepEqual(
    [
      ...new Set(
        map.businessEntityEvidenceNeighborhoods[0].members.map((row) => row.evidenceLineageSha256),
      ),
    ].length,
    1,
  );
  assert.deepEqual(
    map.businessEntityEvidenceNeighborhoods[0].members.map((row) => row.relativePath),
...
test('does not collapse distinct Slack-origin records into one lineage from transport type alone', () => {
  ...
  assert.equal(
    map.businessEntityEvidenceNeighborhoods[0].lineageResolution,
    'byte-identical-proposition-set-plus-transport-origin',
  );
  assert.equal(
    map.businessEntityEvidenceNeighborhoods[0].transportOriginAloneCollapsesLineage,
    false,
  );
});
...
test('persists the map as an immutable object and validates exact replay', () => {
  ...
  const backend = openFileObjectBackend({
    root: mkdtempSync(join(tmpdir(), 'oont-native-object-map-')),
  });
  ...
  const opened = openSourceNativeObjectMap({
    backend,
    nativeObjectMapSha256: materialized.map.nativeObjectMapSha256,
  });
            ...
            fields: [
              {
                ...
                codeUnitStart: content.indexOf('Actual title'),
              },
            ],
          },
        ],
