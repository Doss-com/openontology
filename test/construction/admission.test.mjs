...
import { createConstructionLedgerReader } from '../../dist/construction/admission.js';
...
const fileObjectPath = (objectBackendUri, key) => {
  ...
  return join(
    fileURLToPath(objectBackendUri),
    'objects',
    keySha256.slice(0, 2),
    `${keySha256.slice(2)}.json`,
  );
};
...
function knowledgeRecordPath(f, record) {
  ...
  const descriptor = snapshot?.replayMetadata.blobDescriptors.find(
    (item) => item.logicalPath === pathFor(record.recordSha256),
  );
...
function fixture(t, { protectedHistory = false, namespace = 'example', multiSource = false } = {}) {
  ...
  const buildInput = {
    ...
    ontId: 'construction-admission-example',
    namespace,
    querySchemas: [
      {
    ...
    sources: [
      {
        relativePath: 'tracker/task-1.txt',
        sourceType: 'tracker',
        occurredAt: at(1),
    ...
    nativeObjectInputs: [
      {
        ...
        fields: [
          {
            fieldPath: 'body',
            value: 'AllocationException means allocation mismatch. Status: open.',
          },
          {
            fieldPath: 'status',
            value: 'open',
            propositionFamilyKey: 'task-status',
            validAt: at(1),
            knownAt: at(1),
            businessEntityKeys: ['task:task-1'],
            canonicalProposition: {
              kind: 'OpenOntologySourceNativeCanonicalPropositionV2',
              propositionKey: 'task-1-open',
  ...
  if (multiSource) {
    buildInput.sources.push({
      relativePath: 'tracker/task-2.txt',
      sourceType: 'tracker',
      occurredAt: at(1),
      content: 'Second allocation note. Status: open.',
    });
    buildInput.nativeObjectInputs.push({
      relativePath: 'tracker/task-2.txt',
      ...
      fields: [
        { fieldPath: 'body', value: 'Second allocation note. Status: open.' },
        {
          fieldPath: 'status',
          value: 'open',
          propositionFamilyKey: 'task-status',
  ...
  const witness = {
    ...
    evidence: {
      ...
      textSha256: source.sourceSha256,
    },
  };
  const input = {
    ...
    objectDefs: [
      {
        ...
        source: witness,
        aliases: [{ value: 'allocation mismatch', sourceSystem: 'tracker', source: witness }],
      },
    ],
    claims: [
      {
  ...
  const trust = Object.entries(keys).map(([issuerId, pair]) => ({
    issuerId,
  ...
  function admit(
    ...
    { issuerId = 'reviewer', admittedAt = at(3), targets = [] } = {},
  ) {
  ...
  const read = (extra = {}) =>
    kernel.readSourceNativeConstructionLedger({ options, trustRegistry: trust, ...extra });
  ...
  return {
    ...
    historyBackend: protectedHistory
      ? openCanonicalObjectBackend({ uri: historyBackendUri }).backend
      : undefined,
  };
}
...
function changed(f, { id, name, noAlias = false } = {}) {
  ...
  if (id) {
    input.objectDefs[0].id = id;
    input.claims[0].about = id;
  }
...
function readAtArtifact(f, extra = {}) {
  return kernel.readSourceNativeConstructionLedgerAtArtifact({
    ...
    expectedSourceBinding: sourceBinding(f),
    ...extra,
  });
}
...
function advanceOnlySecondSource(f) {
  ...
  const object = input.nativeObjectInputs.find(
    (item) => item.relativePath === 'tracker/task-2.txt',
  );
  ...
  return materializeSourceNativeObjectOnt({
    backend: f.backend,
    historyBackend: f.historyBackend,
    ontId: f.state.descriptor.ontId,
    branch: f.sourceRoute.branch,
    expectedVersion: before.version,
    sources: materializedInput.sources,
...
function mutateHistory(f, branch, mutate) {
  ...
  f.historyBackend.compareAndSwap(key, {
    ...
    bytes: Buffer.from(
      kernel.stableObjectText({ ...core, historySha256: kernel.stableObjectSha256(core) }),
    ),
  });
}
...
test('only independently signed navigation is written; cold retry preserves source and original bytes', (t) => {
  ...
  assert.deepEqual(
    kernel.validateSourceNativeConstructionAdmissionRecord(JSON.parse(canonical)),
    record,
  );
  ...
  const blob = replay.blobDescriptors.find(
    (item) => item.logicalPath === pathFor(record.recordSha256),
  );
...
test('reviewer labels and signing times cannot claim independence', (t) => {
  const f = fixture(t);
  for (const [issuerId, admittedAt] of [
    ['constructor', at(3)],
    ['reviewer', at(1)],
  ]) {
    assert.throws(
      () =>
        kernel.sourceNativeConstructionAdmissionStatement({
          construction: f.compile(),
          issuerId,
          admittedAt,
        }),
      { code: 'CONSTRUCTION_ADMISSION_INDEPENDENCE' },
    );
  }
  assert.throws(() => f.admit(f.compile(), { admittedAt: 'yesterday' }), {
    code: 'CONSTRUCTION_ADMISSION_TIME',
  });
});
...
for (const [name, mutate] of [
  ...
  [
    'wrong proposer role',
    (f) => {
      f.trust[0].roles = ['reviewer'];
    },
  ],
  [
    'same actual key under two labels',
    (f) => {
      f.keys.reviewer = f.keys.constructor;
...
for (const field of ['signatureBase64', 'proposalSignatureBase64']) {
  test(`structurally valid tampered ${field} never activates or supersedes`, (t) => {
    ...
    const candidate = clone(
      f.admit(changed(f), { admittedAt: at(4), targets: [original.recordSha256] }),
    );
    candidate[field] = Buffer.alloc(64, 7).toString('base64');
    const bad = rehash(candidate);
    assert.equal(
      kernel.validateSourceNativeConstructionAdmissionRecord(bad).recordSha256,
      bad.recordSha256,
    );
...
for (const [name, mutate] of [
  [
    'old proof record kind',
    (r) => {
      r.kind = 'OpenOntologySourceNativeAdmissionRecordV1';
    },
  ],
  [
    'old reviewer statement kind',
    (r) => {
      r.statement.kind = 'OpenOntologySourceNativeAdmissionStatementV1';
    },
  ],
  [
    'old proposer statement kind',
    (r) => {
      r.proposalStatement.kind = 'OpenOntologySourceNativeProposalStatementV1';
    },
  ],
  [
    'proof decision',
    (r) => {
      r.statement.decision = 'admitted';
    },
  ],
  [
    'extra root property',
  ...
  [
    ...
    (r) => {
      r.statement.proofClosed = true;
    },
  ],
  [
    'extra statement symbol',
    (r) => {
      r.statement[Symbol('authority')] = true;
    },
  ],
  [
    'construction substitution',
    (r) => {
      r.construction = { ...r.construction, constructionSha256: digest };
    },
  ],
])
  test(`record validation rejects ${name}`, (t) => {
    const f = fixture(t);
    const record = clone(f.admit());
    mutate(record);
    assert.throws(
      () => kernel.validateSourceNativeConstructionAdmissionRecord(rehash(record)),
      TypeError,
    );
  });
...
test('same proposal multi-reviewer agreement, distinct disjoint batches, and equal names stay separate', (t) => {
  ...
  const records = [
    f.admit(proposal),
    f.admit(proposal, { issuerId: 'reviewer-two' }),
...
test('conflicting constructions are quarantined until correction explicitly names every active conflict', (t) => {
  ...
  const partial = f.admit(changed(f, { noAlias: true }), {
    admittedAt: at(4),
    targets: [original.recordSha256],
  });
  f.write(partial);
  ...
  assert.equal(f.read().activeRecords.length, 0);
  const final = f.admit(changed(f, { noAlias: true }), {
    admittedAt: at(6),
...
test('an overlapping batch is quarantined atomically; corrections cannot drop unrelated IDs', (t) => {
  ...
  const batch = f.admit(f.compile(input));
  f.write(batch);
...
for (const targets of [[digest], [digest, digest], Array(129).fill(digest)]) {
  test(`rejects invalid correction target list length ${targets.length}`, (t) => {
    const f = fixture(t);
    assert.throws(() => f.write(f.admit(f.compile(), { targets })), {
...
test('newer or equal-time targets and different ID sets cannot be superseded', (t) => {
  ...
  const original = f.admit();
  f.write(original);
  for (const [construction, admittedAt] of [
    [changed(f), at(3)],
    [changed(f), at(2)],
    [changed(f, { id: 'different' }), at(4)],
  ]) {
    assert.throws(
      () => f.write(f.admit(construction, { admittedAt, targets: [original.recordSha256] })),
      { code: 'CONSTRUCTION_ADMISSION_SUPERSESSION' },
    );
  }
});
...
test('cold replay rejects planted dangling or out-of-scope corrections and preserves the valid map', (t) => {
  ...
  f.plant(
    f.admit(changed(f, { id: 'different' }), {
      admittedAt: at(4),
      targets: [original.recordSha256],
    }),
  );
...
test('correction cannot target a structurally valid record from another binding', (t) => {
  ...
  const { constructionSha256: _hash, ...core } = proposal;
  proposal.constructionSha256 = kernel.stableObjectSha256(core);
  const foreign = f.admit(proposal);
  f.plant(foreign);
...
test('no source-cut substitution or cross-cut correction even when bytes and names match', (t) => {
  ...
  assert.throws(() => f.write(original, { options: nextOptions }), {
    code: 'SEMANTIC_CONSTRUCTION_BINDING',
  });
  const nextState = kernel.openProductState(nextOptions);
  const input = clone(f.input);
  input.proposedAt = at(5);
  const witness = {
    ...input.objectDefs[0].source,
    nativeObjectSha256: nextState.objectOnt.map.nativeObjects[0].nativeObjectSha256,
  };
  input.objectDefs[0].source = witness;
  input.objectDefs[0].aliases[0].source = witness;
  input.claims[0].source = witness;
  ...
  assert.equal(
    kernel.validateSourceNativeConstructionAdmissionRecord(original).recordSha256,
...
for (const [name, mutate, alternatePath] of [
  ...
  test(`cold ledger degrades for ${name} without poisoning a valid disjoint batch`, (t) => {
    ...
    const original = f.admit();
    f.write(original);
    const bad = clone(f.admit(changed(f, { id: 'other' })));
    mutate(bad);
...
test('knowledge cannot write onto the source branch, and custom branches stay explicit', (t) => {
  const f = fixture(t);
  const record = f.admit();
  assert.throws(() => f.write(record, { knowledgeBranch: 'main' }), {
    code: 'CONSTRUCTION_ADMISSION_BRANCH',
  });
...
test('canonical source-envelope corruption propagates through repeated and standalone ledger reads', (t) => {
  ...
  assert.throws(
    () =>
      kernel.readSourceNativeConstructionLedger({
        options: f.options,
        trustRegistry: f.trust,
      }),
    isBackendCorruption,
...
test('internal reader keeps its commit floor through missing and rewound metadata, then recovers to a descendant', (t) => {
  ...
  const pinnedStore = {
    ...sourceStore,
    readRefMetadataSnapshot: (input) =>
      forcedSnapshot === undefined ? sourceStore.readRefMetadataSnapshot(input) : forcedSnapshot,
  };
  const reader = createConstructionLedgerReader(
    {
      descriptor: f.state.descriptor,
  ...
  assert.deepEqual(
    recovered.activeRecords.map((record) => record.recordSha256).sort(),
...
test('readSnapshot exposes active agreement, correction and conflict states without raw records', (t) => {
  ...
  f.write(agreement);
  f.write(agreementBySecondReviewer);
  ...
  assert.deepEqual(
    snapshot.records.map((record) => record.state),
    ['active', 'active'],
  );
  assert(
    snapshot.records.every(
      (record) => record.recordSha256 && record.blobSha256.startsWith('sha256:'),
    ),
  );
  assert(
    snapshot.records.every((record) => !('construction' in record) && !('statement' in record)),
  );

  const correction = f.admit(changed(f, { noAlias: true }), {
    admittedAt: at(4),
    targets: [agreement.recordSha256, agreementBySecondReviewer.recordSha256],
  });
  f.write(correction);
  snapshot = createConstructionLedgerReader(f.state, f.trust).readSnapshot();
  const corrected = snapshot.records.find(
    (record) => record.recordSha256 === correction.recordSha256,
  );
  ...
  for (const target of [agreement, agreementBySecondReviewer]) {
    const disposition = snapshot.records.find(
      (record) => record.recordSha256 === target.recordSha256,
    );
  ...
  assert(
    conflicting.every((record) => record.conflictingObjectDefIds.includes('allocation-exception')),
  );
...
test('superseded records do not inherit current conflict annotations', (t) => {
  ...
  assert(
    currentConflicts.every((record) =>
      record.conflictingObjectDefIds.includes('allocation-exception'),
    ),
  );
});
...
test('a larger planted conflict group keeps per-record conflict metadata linear and bounded', (t) => {
  ...
  assert(
    snapshot.records.every(
      (record) =>
        record.conflictingObjectDefIds.length === 1 &&
...
test('readSnapshot keeps ineligible history distinct from supersession and source binding', (t) => {
  ...
  const original = f.admit();
  f.write(original);
  const revoked = f.read({
    trustRegistry: f.trust.filter((entry) => entry.issuerId !== 'reviewer'),
  });
  assert.equal(revoked.activeRecords.length, 0);
  const revokedSnapshot = createConstructionLedgerReader(
    f.state,
    f.trust.filter((entry) => entry.issuerId !== 'reviewer'),
  ).readSnapshot();
  const revokedRecord = revokedSnapshot.records.find(
    (record) => record.recordSha256 === original.recordSha256,
  );
  ...
  const correctedSnapshot = createConstructionLedgerReader(
    f.state,
    f.trust.filter((entry) => entry.issuerId !== 'reviewer'),
  ).readSnapshot();
  const ineligibleTarget = correctedSnapshot.records.find(
    (record) => record.recordSha256 === original.recordSha256,
  );
  ...
  assert.equal(
    correctedSnapshot.records.find((record) => record.recordSha256 === correction.recordSha256)
      ?.state,
    'active',
  );
  ...
  const bindingRecord = bindingSnapshot.records.find(
    (record) => record.blobSha256 === blob.storedSha256,
  );
...
test('malformed core hash cannot overwrite a valid record disposition', (t) => {
  ...
  const malformed = snapshot.records.find(
    (record) => record.blobSha256 === malformedBlob.storedSha256 && record.recordSha256 === null,
  );
...
test('reader diagnostics normalize arbitrary backend codes in record and history failures', (t) => {
  ...
  assert(
    recordSnapshot.records.some((record) =>
      record.reasonCodes.includes('CONSTRUCTION_ADMISSION_RECORD'),
    ),
  );
  assert(recordSnapshot.ledger.diagnosticCodes.every((code) => !code.includes('SECRET')));
  assert(
    recordSnapshot.records.every((record) =>
      record.reasonCodes.every((code) => !code.includes('SECRET')),
    ),
  );
  ...
  const historyReader = createConstructionLedgerReader(
    { ...f.state, store: historyStore },
    f.trust,
  );
...
test('readSnapshot returns no cached records for missing or rewound history and recovers at a descendant', (t) => {
  ...
  const reader = createConstructionLedgerReader(
    {
      ...
      store: {
        ...
        readRefMetadataSnapshot: (input) =>
          forcedSnapshot === undefined
            ? sourceStore.readRefMetadataSnapshot(input)
            : forcedSnapshot,
      },
    },
    f.trust,
  ...
  assert.deepEqual(
    recovered.records.map((record) => record.recordSha256).sort(),
...
for (const historicalFirst of [false, true]) {
  test(`mixed current/historical reads preserve the rollback floor, historical first: ${historicalFirst}`, (t) => {
    ...
    const reader = createConstructionLedgerReader(
      {
        ...
        store: {
          ...sourceStore,
          readRefMetadataSnapshot: (input) =>
            input.branch === f.branch && forcedSnapshot !== undefined
              ? forcedSnapshot
              : sourceStore.readRefMetadataSnapshot(input),
        },
      },
    ...
    const selected = {
      commitSha256: earlier.ref.commitSha256,
      replaySha256: earlier.ref.replaySha256,
...
test('historical ledger read accepts a protected descendant and returns the complete A construction', (t) => {
  ...
  assert.notEqual(
    currentB.objectOnt.sources[1].sourceSha256,
    f.state.objectOnt.sources[1].sourceSha256,
  );
  ...
  assert.deepEqual(
    historical.activeRecords[0].construction.objectDefs,
    record.construction.objectDefs,
  );
...
test('historical read refuses a source selection that advances during signed record delivery', (t) => {
  ...
  f.write(record);
  let descendant;
  const result = interleaveReadFileSync(
    knowledgeRecordPath(f, record),
    () => true,
    () => {
  ...
  assert.equal(
    currentB.objectOnt.sources[1].content,
...
test('historical read refuses a knowledge correction that arrives during signed record delivery', (t) => {
  ...
  const result = interleaveReadFileSync(
    knowledgeRecordPath(f, original),
    () => true,
    () => {
      f.write(correction);
    },
...
test('historical read refuses stable empty knowledge becoming present between protected snapshots', (t) => {
  ...
  const record = f.admit();
  assert.equal(f.state.store.readRefMetadata(f.route), null);
  const sourceHistoryPath = fileObjectPath(
    f.historyBackendUri,
  ...
  assert.equal(
    observedError?.code,
    'CONSTRUCTION_ADMISSION_CONCURRENT',
...
test('a B artifact does not inherit A knowledge records on its default branch', (t) => {
  ...
  kernel.buildSourceNativeProduct({
    artifactRoot: bOptions.artifactRoot,
    objectBackendUri: f.objectBackendUri,
    historyBackendUri: f.historyBackendUri,
    input,
  });
  const currentB = kernel.readSourceNativeConstructionLedger({
    options: bOptions,
    trustRegistry: f.trust,
  });
  ...
  assert.throws(
    () =>
      kernel.readSourceNativeConstructionLedgerAtArtifact({
        options: bOptions,
        trustRegistry: f.trust,
        expectedSourceBinding: sourceBinding(f),
      }),
    ...
    () =>
      kernel.readSourceNativeConstructionLedgerAtArtifact({
        ...
        expectedSourceBinding: {
          ontId: resealed.ontId,
          namespace: resealed.namespace,
          artifactSha256: resealed.artifactSha256,
          sourceCommitSha256: resealed.sourceCommitSha256,
          sourceReplaySha256: resealed.sourceReplaySha256,
          sourceCatalogSha256: resealed.sourceCatalogSha256,
          nativeObjectMapSha256: resealed.nativeObjectMapSha256,
        },
      }),
    { code: 'CONSTRUCTION_ADMISSION_BRANCH' },
  );
...
test('historical read requires protected v2 history and an exact seven-field source binding', (t) => {
  ...
  const mutations = [
    ...
    [
      'wrong hash',
      (value) => {
        value.artifactSha256 = 'sha256:bad';
      },
    ],
    [
      'mismatched cut',
      (value) => {
        value.sourceCommitSha256 = digest;
      },
    ],
  ];
  for (const [name, mutate] of mutations) {
    ...
    assert.throws(
      () =>
        kernel.readSourceNativeConstructionLedgerAtArtifact({
          options: protectedFixture.options,
          trustRegistry: protectedFixture.trust,
...
test('stable absent knowledge is empty, while missing, corrupt, and pending protected history refuse', (t) => {
  ...
  unlinkSync(
    fileObjectPath(
      missingSource.historyBackendUri,
  ...
  corruptKnowledge.historyBackend.compareAndSwap(knowledgeKey, {
    expectedVersion: knowledgeHead.version,
  ...
  const pendingBaseVersion = pendingSource.state.store.readRefMetadata(
    pendingSource.sourceRoute,
  ).version;
...
test('old signed query proofs and new construction share the branch without reinterpretation', async (t) => {
  ...
  const bundle = await kernel.compileSourceNativeSemanticKnowledgeBundle({
    options: f.options,
    query,
    proposedBy: 'constructor',
    proposedAt: at(2),
  ...
  const statement = kernel.sourceNativeAdmissionStatement({
    bundle,
    issuerId: 'reviewer',
    admittedAt: at(3),
  });
  const oldRecord = kernel.compileSourceNativeAdmissionRecord({
    bundle,
    proposalStatement,
    statement,
  ...
  const oldWrite = kernel.writeSourceNativeAdmittedKnowledge({
    options: f.options,
    record: oldRecord,
    trustRegistry: f.trust,
  });
  ...
  const proofClient = kernel.openSourceNativeProductWithAdmittedKnowledge(f.options, {
    trustRegistry: f.trust,
  });
  ...
  const oldBlob = latest.replayMetadata.blobDescriptors.find((item) =>
    item.logicalPath.endsWith(`${oldRecord.recordSha256.slice(7)}.json`),
  );
  ...
  const alternateStatement = kernel.sourceNativeAdmissionStatement({
    bundle,
    issuerId: 'reviewer-two',
    admittedAt: at(3),
  });
  const oldSecond = kernel.compileSourceNativeAdmissionRecord({
    bundle,
    proposalStatement,
  ...
  kernel.writeSourceNativeAdmittedKnowledge({
    options: f.options,
    record: oldSecond,
    trustRegistry: f.trust,
  });
