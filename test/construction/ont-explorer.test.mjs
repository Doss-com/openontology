...
function fixture(
  t,
  {
    names = ['AllocationException', 'DocumentedTask'],
  ...
  const baseInput = (
    definitions = names.map((name, index) => ({ id: `concept-${index}`, name })),
  ) => ({
    ...
    objectDefs: definitions.map((definition) => {
      const aliases =
        definition.aliases ?? (definition.alias === undefined ? [] : [definition.alias]);
      return {
        kind: 'ObjectDef',
        id: definition.id,
    ...
    claims: definitions.map((definition, index) => ({
      kind: 'Claim',
      id: `claim-${definition.id}-${index}`,
      about: definition.id,
      predicate: index === 0 ? 'defines' : 'mentions',
  ...
  const plant = (value, logicalPath = `${prefix}${value.recordSha256.slice(7)}.json`) => {
    ...
    const blob = state.store.putBlob({
      logicalPath,
      bytes: Buffer.from(kernel.stableObjectText(value)),
      mediaType: 'application/json',
    });
    const commit = state.store.writeCommitMetadata({
      ontId: route.ontId,
      parents: [parent],
      ontManifest: state.store.readCommit(parent).commit.ontManifest,
      blobs: [blob],
    });
  ...
  const openConstruction = (config = configuration) =>
    kernel.openSourceNativeProductWithConstruction(options, config);
  return {
    root,
    options,
    state,
...
for (const historical of [false, true]) {
  for (const method of ['nodes', 'edges', 'records', 'status', 'read']) {
    test(`${historical ? 'historical' : 'current'} ${method} rejects knowledge publication before egress`, (t) => {
      ...
      const explorer = historical
        ? f.open({
            ...f.configuration,
            snapshot: page.binding,
      ...
      const successor = f.admit(
        f.compile(f.baseInput([{ id: 'later-concept', name: 'DocumentedTask' }])),
        { day: 4 },
      );
      interleavePublication(
        t,
        f,
        () => f.write(successor),
...
for (const historical of [false, true]) {
  for (const method of ['nodes', 'edges', 'records', 'status', 'read']) {
    test(`${historical ? 'historical' : 'current'} ${method} rejects source publication before egress`, (t) => {
      ...
      const explorer = historical
        ? f.open({ ...f.configuration, snapshot, recordSha256: record.recordSha256 })
        : current;
      const passage = explorer.nodes().nodes.find((node) => node.kind === 'passage');
      assert(passage?.readRef);
      ...
      const publish = () =>
        kernel.buildSourceNativeProduct({
          artifactRoot: join(f.root, 'successor'),
          input,
...
test('source-only Ont exposes native nodes, identity, coverage and unknown freshness', (t) => {
  ...
  assert(
    scoped.nodes.every(
      (node) =>
        node.kind === 'native-object' && node.nativeObject?.identity.sourceSystem === 'clickup',
    ),
  );
});
...
test('canonical graph preserves Claim IDs and passage-to-ObjectDef direction', (t) => {
  ...
  const nodes = allPages(
    () => explorer.nodes({ limit: 64 }),
    (cursor) => explorer.nodes({ cursor, limit: 64 }),
    'nodes',
  ).items;
  const edges = allPages(
    () => explorer.edges({ limit: 128 }),
    (cursor) => explorer.edges({ cursor, limit: 128 }),
    'edges',
  ).items;
  const objectDef = nodes.find(
    (node) => node.kind === 'object-def' && node.objectDef?.id === 'allocation-exception',
  );
  assert(objectDef);
  const claim = edges.find(
    (edge) => edge.kind === 'claim' && edge.about === 'allocation-exception',
  );
  assert(claim);
  ...
  assert.equal(
    Buffer.byteLength(currentExact.exactText),
...
test('reviewer agreement is deduplicated while same-name distinct definitions remain separate', (t) => {
  ...
  const first = f.admit(
    f.compile(f.baseInput([{ id: 'same-a', name: 'SharedConcept', alias: 'allocation mismatch' }])),
  );
  const secondReviewer = f.admit(
    f.compile(f.baseInput([{ id: 'same-a', name: 'SharedConcept', alias: 'allocation mismatch' }])),
    { issuerId: 'reviewer-two' },
  );
  const distinct = f.admit(f.compile(f.baseInput([{ id: 'same-b', name: 'SharedConcept' }])), {
    day: 4,
  });
  f.write(first);
  f.write(secondReviewer);
  f.write(distinct);
  const explorer = f.open();
  ...
  assert.deepEqual(
    graphPage.nodes
      .filter((node) => node.kind === 'object-def')
      .map((node) => node.objectDef.id)
      .sort(),
    ['same-a', 'same-b'],
  );
  const edges = allPages(
    () => explorer.edges(),
    (cursor) => explorer.edges({ cursor }),
    'edges',
  ).items;
  ...
  assert.equal(
    records.records.filter(
      (record) => record.constructionSha256 === first.construction.constructionSha256,
    ).length,
    2,
  );
  ...
  assert.doesNotThrow(() =>
    explorer.nodes({ term: 'sharedconcept', cursor: cursorPage.nextCursor, limit: 1 }),
  );
});

test('scoped aliases, explicit IDs and one-hop focus keep identities distinct', async (t) => {
  const f = fixture(t, { names: ['ScopedConcept'] });
  const construction = f.admit(
    f.compile(
      f.baseInput([{ id: 'scoped-concept', name: 'ScopedConcept', alias: 'allocation mismatch' }]),
    ),
  );
  ...
  const scopedNodes = explorer.nodes({
    scope: { sourceSystem: 'clickup', objectType: 'ClickupTask' },
  });
  ...
  assert(
    scopedNodes.nodes
      .filter((node) => node.kind === 'native-object')
      .every((node) => node.nativeObject?.identity.sourceSystem === 'clickup'),
  );
  assert.equal(
    explorer.nodes({ term: 'allocation mismatch', scope: { sourceSystem: 'docs' } }).totalCount,
    0,
  );
  const mismatchedAlias = f.admit(
    f.compile(
      f.baseInput([
        {
          id: 'mismatched-alias',
          name: 'ScopedConcept',
          alias: 'allocation mismatch',
          aliasSource: 'docs/guide.txt',
          claimSource: 'clickup/CT-17.txt',
        },
      ]),
    ),
    { day: 4 },
  ...
  const scopedAlias = afterMismatch.nodes({
    term: 'allocation mismatch',
    scope: { sourceSystem: 'clickup', objectType: 'ClickupTask' },
  });
  ...
  const objectDef = explorer
    .nodes({ term: 'ScopedConcept' })
    .nodes.find((node) => node.kind === 'object-def');
  assert(objectDef);
  ...
  assert.throws(() => explorer.nodes({ ids: [objectDef.id], focusId: objectDef.id }), {
    code: 'SOURCE_NATIVE_EXPLORER_INPUT',
  });
  assert.throws(() => explorer.nodes({ focusId: 'object-def:unknown' }), {
    code: 'SOURCE_NATIVE_EXPLORER_FOCUS',
  });
});
...
test('node, edge and record pages exhaust with bounded counts and bytes', (t) => {
  ...
  const admission = f.admit(
    f.compile(f.baseInput(names.map((name, index) => ({ id: `concept-${index}`, name })))),
  );
  f.write(admission);
  const explorer = f.open();
  const nodePages = allPages(
    () => explorer.nodes({ limit: 7 }),
    (cursor) => explorer.nodes({ limit: 7, cursor }),
    'nodes',
  );
  const edgePages = allPages(
    () => explorer.edges({ limit: 11 }),
    (cursor) => explorer.edges({ limit: 11, cursor }),
    'edges',
  );
  const recordPages = allPages(
    () => explorer.records({ limit: 1 }),
    (cursor) => explorer.records({ limit: 1, cursor }),
    'records',
  );
  const status = explorer.status();
  ...
  assert(
    recordPages.items.some((record) => record.constructionCoverage?.examinedSourceCount === 3),
  );
});
...
test('boundary-sized valid metadata pages include cursor overhead in their byte bound', (t) => {
  ...
  const aliases = Array.from(
    { length: 16 },
    (_, aliasIndex) => `BoundaryAlias${aliasIndex}-${'a'.repeat(220)}`,
  );
  ...
  assert(
    definitions.every((definition) =>
      f.sourceByPath.get('docs/guide.txt').content.includes(definition.name),
    ),
  );
  ...
  const pages = allPages(
    () => explorer.nodes({ limit: 64 }),
    (cursor) => explorer.nodes({ limit: 64, cursor }),
    'nodes',
  );
...
test('a valid oversized native identity refuses as one item without an empty cursor page', (t) => {
  ...
  const nativeObject = f.state.objectOnt.map.nativeObjects.find(
    (object) => object.objectIdentity.externalId === longExternalId,
  );
  assert(nativeObject);
...
test('records retain conflict, correction and ineligible states without entering current graph', (t) => {
  ...
  const alternate = f.admit(f.compile(f.baseInput([{ id: 'correctable', name: 'Alternate' }])), {
    day: 4,
  });
  f.write(original);
  f.write(alternate);
  ...
  assert.equal(
    conflictExplorer.nodes({ limit: 64 }).nodes.filter((node) => node.kind === 'object-def').length,
    0,
  );
  const correction = f.admit(
    f.compile(f.baseInput([{ id: 'correctable', name: 'CorrectableNow' }])),
    {
      day: 5,
      targets: [original.recordSha256, alternate.recordSha256],
    },
  );
  f.write(correction);
  ...
  const revoked = f.open({
    trustRegistry: f.trustRegistry.filter((entry) => entry.issuerId !== 'reviewer'),
  });
  ...
  assert.equal(
    revoked.nodes({ limit: 64 }).nodes.filter((node) => node.kind === 'object-def').length,
    0,
  );
});
...
test('malformed construction metadata remains bounded and valid graph items survive', (t) => {
  ...
  assert.equal(
    explorer.nodes({ limit: 64 }).nodes.filter((node) => node.kind === 'object-def').length,
    2,
  );
});
...
test('foreign and filter-mismatched cursors are refused, and projection changes stale cursors', (t) => {
  ...
  assert.throws(() => b.nodes({ cursor: first.nextCursor, limit: 1 }), {
    code: 'SOURCE_NATIVE_EXPLORER_CURSOR',
  });
  assert.throws(
    () => a.nodes({ cursor: first.nextCursor, term: 'AllocationException', limit: 1 }),
    { code: 'SOURCE_NATIVE_EXPLORER_CURSOR_STALE' },
  );
  const record = f.admit(f.compile(f.baseInput([{ id: 'new-concept', name: 'NewConcept' }])), {
    day: 4,
  });
  f.write(record);
  assert.throws(() => a.nodes({ cursor: first.nextCursor, limit: 1 }), {
    code: 'SOURCE_NATIVE_EXPLORER_CURSOR',
  });
});
...
test('pinned reviewer configuration and source cut stay stable after caller mutation', (t) => {
  ...
  assert.equal(
    after.binding.reviewerConfigurationSha256,
    before.binding.reviewerConfigurationSha256,
  );
...
for (const [protectedHistory, historical] of [
  [false, false],
  [false, true],
  [true, false],
  [true, true],
  ...
  test(`a removed source head invalidates every ${historical ? 'historical' : 'current'} operation, protected: ${protectedHistory}`, (t) => {
    ...
    const code = protectedHistory
      ? 'OBJECT_ONT_HISTORY_MISMATCH'
      : historical
        ? 'CONSTRUCTION_ADMISSION_MISSING'
        : 'SOURCE_NATIVE_EXPLORER_CONCURRENT';
    for (const { method, explorer, ref } of sessions) {
      assert.throws(
        () => explorer[method](method === 'read' ? { ref } : undefined),
        { code },
        method,
      );
    ...
    if (protectedHistory) f.state.store.recoverRefHistory(route);
      ...
      f.backend.compareAndSwap(head.key, {
...
test('reviewer configuration binds public key identity and edge IDs ignore agreement representative', (t) => {
  ...
  const second = f.admit(f.compile(f.baseInput([{ id: 'stable', name: 'StableConcept' }])), {
    issuerId: 'reviewer-two',
    day: 4,
  });
  ...
  assert.equal(
    explorer.status().binding.reviewerConfigurationSha256,
    reversed.status().binding.reviewerConfigurationSha256,
  );
  assert.deepEqual(
    beforeIds,
    explorer.edges({ limit: 128 }).edges.map((edge) => edge.id),
  );
  const rotated = generateKeyPairSync('ed25519');
  const rotatedRegistry = f.trustRegistry.map((entry) =>
    entry.issuerId === 'reviewer'
      ? { ...entry, publicKeyPem: rotated.publicKey.export({ type: 'spki', format: 'pem' }) }
      : entry,
  );
  assert.notEqual(
    explorer.status().binding.reviewerConfigurationSha256,
...
test('cursor retention is bounded and evicts the oldest cursor', (t) => {
  ...
  assert.throws(() => explorer.nodes({ cursor: first.nextCursor, limit: 1 }), {
    code: 'SOURCE_NATIVE_EXPLORER_CURSOR',
  });
});
...
test('history failure suppresses cached pages and recovery restores the same reader', (t) => {
  const f = fixture(t, {
    ...
    names: ['AllocationException', 'RecoveredConcept'],
  });
  const original = f.admit(
    f.compile(f.baseInput([{ id: 'concept-0', name: 'AllocationException' }])),
  );
  f.write(original);
  const explorer = f.open();
  assert.equal(explorer.status().state, 'ready');
  ...
  const correction = f.admit(
    f.compile(f.baseInput([{ id: 'concept-0', name: 'RecoveredConcept' }])),
  ...
  assert.throws(() => explorer.nodes({ cursor: stale.nextCursor, limit: 1 }), {
    code: 'SOURCE_NATIVE_EXPLORER_CURSOR',
  });
...
test('unknown and unsupported explorer inputs refuse before page projection', (t) => {
  ...
  assert.throws(() => explorer.nodes({ ids: Array.from({ length: 65 }, () => 'x') }), {
    code: 'SOURCE_NATIVE_EXPLORER_INPUT',
  });
  assert.throws(() => explorer.records({ state: 'ready' }), {
    code: 'SOURCE_NATIVE_EXPLORER_INPUT',
  });
  assert.throws(() => explorer.edges({ focusId: 'not-a-node' }), {
    code: 'SOURCE_NATIVE_EXPLORER_FOCUS',
  });
...
test('a selected record opens bounded historical pages and exact offered passages', (t) => {
  ...
  const admission = f.admit(
    f.compile(
      f.baseInput([{ id: 'historical', name: 'HistoricalConcept', alias: 'allocation mismatch' }]),
    ),
  );
  f.write(admission);
  const current = f.open();
  const currentRecords = current.records({ limit: 64 });
  const selected = currentRecords.records.find(
    (item) => item.recordSha256 === admission.recordSha256,
  );
  assert(selected);
  assert.equal(typeof current.read, 'function');

  const history = f.open({
    ...f.configuration,
    snapshot: currentRecords.binding,
    recordSha256: selected.recordSha256,
  });
  const nodePages = allPages(
    () => history.nodes({ limit: 7 }),
    (cursor) => history.nodes({ limit: 7, cursor }),
    'nodes',
  );
  const edgePages = allPages(
    () => history.edges({ limit: 11 }),
    (cursor) => history.edges({ limit: 11, cursor }),
    'edges',
  );
  const recordPages = allPages(
    () => history.records({ limit: 1 }),
    (cursor) => history.records({ limit: 1, cursor }),
    'records',
  );
  ...
  assert.equal(
    exact.receiptSha256,
    kernel.stableObjectSha256({ ...exact, receiptSha256: undefined }),
  );

  const successor = f.admit(
    f.compile(f.baseInput([{ id: 'successor', name: 'HistoricalConcept' }])),
    { day: 4 },
  );
  f.write(successor);
...
test('historical selection rejects incomplete bindings and preserves safe refusals', (t) => {
  ...
  assert.throws(
    () => f.open({ ...f.configuration, snapshot: page.binding, recordSha256: wrongRecord }).nodes(),
    { code: 'CONSTRUCTION_ADMISSION_RECORD' },
  );
  const wrongSource = { ...page.binding, sourceCommitSha256: `sha256:${'f'.repeat(64)}` };
  assert.throws(
    () =>
      f.open({ ...f.configuration, snapshot: wrongSource, recordSha256: selected.recordSha256 }),
    { code: 'SOURCE_NATIVE_EXPLORER_BINDING' },
  );
  const wrongReviewer = {
    ...f.configuration,
    trustRegistry: f.trustRegistry.slice(1),
    snapshot: page.binding,
    recordSha256: selected.recordSha256,
  };
  ...
  const invalid = afterMalformed
    .records({ limit: 64 })
    .records.find((item) => item.recordSha256 === null);
  assert(invalid);
  const malformedHistory = f.open({
    ...f.configuration,
    snapshot: afterMalformed.records().binding,
...
test('current offered passages become stale when active knowledge changes', (t) => {
  const f = fixture(t, {
    names: ['CurrentConcept', 'CorrectedConcept'],
    extraText: ' multibyte λ',
  });
  ...
  assert(passage?.readRef);
  const correction = f.admit(
    f.compile(f.baseInput([{ id: 'current', name: 'CorrectedConcept' }])),
    {
      day: 4,
...
test('historical inspection rejects every mismatched snapshot identity field', (t) => {
  ...
  for (const key of [
    ...
    const changed = {
      ...snapshot,
      [key]: key.endsWith('Sha256') ? `sha256:${'f'.repeat(64)}` : 'other',
    };
    assert.throws(
      () => f.open({ ...f.configuration, snapshot: changed, recordSha256: record.recordSha256 }),
      { code: 'SOURCE_NATIVE_EXPLORER_BINDING' },
      key,
    );
  }
  for (const key of ['knowledgeCommitSha256', 'knowledgeReplaySha256']) {
    const history = f.open({
      ...f.configuration,
      snapshot: { ...snapshot, [key]: `sha256:${'f'.repeat(64)}` },
      recordSha256: record.recordSha256,
    });
    assert.throws(() => history.nodes(), { code: 'CONSTRUCTION_ADMISSION_BRANCH' }, key);
  }
  const wrongBranch = f.open({
    ...f.configuration,
    snapshot: { ...snapshot, knowledgeBranch: 'other' },
    recordSha256: record.recordSha256,
  });
...
test('offered reads reuse handles and evict only after 1,024 distinct passages', (t) => {
  ...
  const ids = [
    ...new Set(
      edges.flatMap((edge) => [edge.from, edge.to]).filter((id) => id.startsWith('passage:')),
    ),
  ].sort();
  ...
  assert.throws(() => explorer.read({ ref: first.readRef }), {
    code: 'SOURCE_NATIVE_EXPLORER_REFERENCE',
  });
...
test('exact multibyte reads reach 64 KiB and invalid or oversized spans never enter the graph', (t) => {
  ...
  const setSpan = (byteEnd) => {
    ...
    Object.assign(value.objectDefs[0].source.evidence, {
      byteStart,
      byteEnd,
  ...
  assert.throws(() => f.compile(setSpan(byteStart + 65538)), {
    code: 'SEMANTIC_CONSTRUCTION_SPAN',
  });
  assert.throws(() => f.compile(setSpan(byteStart + 65535)), {
  ...
  const history = f.open({
    ...f.configuration,
    snapshot: current.records().binding,
    recordSha256: record.recordSha256,
  });
...
test('a correction-ineligible authenticated record is historical but readable by explicit binding', (t) => {
  ...
  const original = f.admit();
  f.write(original);
  const invalidCorrection = f.admit(f.compile(), { day: 4, targets: [`sha256:${'e'.repeat(64)}`] });
...
test('conflicting and superseded records remain inspectable only through explicit history', (t) => {
  const f = fixture(t, {
    protectedHistory: true,
    names: ['Correctable', 'Alternate', 'CorrectableNow'],
  });
  const original = f.admit(f.compile(f.baseInput([{ id: 'correctable', name: 'Correctable' }])));
  const alternate = f.admit(f.compile(f.baseInput([{ id: 'correctable', name: 'Alternate' }])), {
    day: 4,
  ...
  const conflictHistory = f.open({
    ...f.configuration,
    snapshot: conflictRecords.binding,
    recordSha256: original.recordSha256,
  });
  ...
  assert(
    conflictNodes.nodes.some(
      (node) => node.kind === 'object-def' && node.objectDef?.id === 'correctable',
    ),
  );
  ...
  const historical = f.open({
    ...f.configuration,
    snapshot: superseded.binding,
    recordSha256: original.recordSha256,
  });
  const oldNodes = historical.nodes({ term: 'Correctable', limit: 64 });
  assert(
    oldNodes.nodes.some(
      (node) => node.kind === 'object-def' && node.objectDef?.name === 'Correctable',
    ),
  );
  ...
  assert(passage?.readRef);
  assert.equal(
    historical.read({ ref: passage.readRef }).binding.admissionRecordSha256,
...
test('historical inspection refuses a protected knowledge rewind and recovers explicitly', (t) => {
  ...
  const row = current
    .records({ limit: 64 })
    .records.find((item) => item.recordSha256 === original.recordSha256);
  assert(row);
  const history = f.open({
    ...f.configuration,
    snapshot: current.records().binding,
  ...
  const correction = f.admit(
    f.compile(f.baseInput([{ id: 'concept-0', name: 'CorrectedConcept' }])),
    { day: 4, targets: [original.recordSha256] },
  );
  f.write(correction);
  const advanced = f.state.store.readRefMetadata(f.route);
  f.backend.compareAndSwap(advanced.key, {
