...
function fixture(t, { taskCount = 2, protectedHistory = false } = {}) {
  ...
  const sources = [{ relativePath: 'docs/guide.txt', sourceType: 'docs', occurredAt: at(1),
    content: 'AllocationException means inventory allocation mismatch. The ClickUp label is allocation mismatch.' },
  ...Array.from({ length: taskCount }, (_, index) => ({ relativePath: `clickup/CT-${17 + index}.txt`,
    sourceType: 'clickup', occurredAt: at(1),
  ...
  const buildInput = {
    ...
    ontId: 'construction-navigation-example', namespace: 'example', sources,
    querySchemas: [
    ...
    nativeObjectInputs: sources.map((source, index) => ({ relativePath: source.relativePath,
      objectIdentity: { home: 'ObjectDef/InstanceRef', sourceSystem: source.sourceType,
        objectType: index ? 'ClickupTask' : 'Document', namespace: 'example', externalId: index ? `CT-${16 + index}` : 'guide' },
      fields: [{ fieldPath: 'body', value: source.content },
        ...(index ? [{ fieldPath: 'status', value: index === 1 ? 'open' : 'closed' }] : [])],
    })),
  };
  kernel.buildSourceNativeProduct({ ...options, objectBackendUri, input: buildInput,
  ...
  const witness = path => {
    ...
    return { nativeObjectSha256: object.nativeObjectSha256, evidence: {
      sourceRef: path, sourceSha256: object.sourceSha256, byteStart: 0,
  ...
  const input = { proposedBy: 'constructor', proposedAt: at(2), method: 'authored',
    objectDefs: [{ kind: 'ObjectDef', id: 'allocation-exception', name: 'AllocationException',
      source: witness('docs/guide.txt'), aliases: [{ value: 'allocation mismatch', sourceSystem: 'clickup', source: witness('docs/guide.txt') }] }],
    claims: [{ kind: 'Claim', id: 'definition', about: 'allocation-exception', predicate: 'defines', source: witness('docs/guide.txt') },
      ...sources.slice(1).map((source, i) => ({ kind: 'Claim', id: `mention-${i}`, about: 'allocation-exception',
        predicate: 'mentions', source: witness(source.relativePath) }))],
  ...
  const trustRegistry = Object.entries(keys).map(([issuerId, pair]) => ({ issuerId,
    publicKeyPem: pair.publicKey.export({ type: 'spki', format: 'pem' }), roles: [issuerId === 'constructor' ? 'proposer' : 'reviewer'] }));
  ...
  function record(construction = compile(), { issuerId = 'reviewer', day = 3, targets = [] } = {}) {
    ...
    const statement = kernel.sourceNativeConstructionAdmissionStatement({ construction, issuerId, admittedAt: at(day), supersedesRecordSha256s: targets });
    return kernel.compileSourceNativeConstructionAdmissionRecord({ construction, proposalStatement, statement,
      proposalSignatureBase64: signature(proposalStatement, keys.constructor.privateKey), signatureBase64: signature(statement, keys[issuerId].privateKey) });
  }
  const write = entry => kernel.writeSourceNativeConstructionAdmission({ options, trustRegistry, record: entry });
  const open = (config = configuration, lifecycle = null) => kernel.openSourceNativeProductWithConstruction(options, config, lifecycle);
  const route = { ontId: state.descriptor.ontId, branch: `knowledge-${state.objectOnt.commitSha256.slice(7, 23)}` };
  return { root, options, objectBackendUri, state, input, buildInput, configuration, trustRegistry,
    compile, record, write, open, witness, route, backend: openCanonicalObjectBackend({ uri: objectBackendUri }).backend };
}

test('cold concept discovery, exact read, and explicit native verification use one client', async t => {
  const f = fixture(t); const record = f.record(); f.write(record);
  const ont = f.open();
  ...
  assert(result.matches.every(match => match.requiredForProof === false && match.nativeObject.sourceSystem === 'clickup'));
  ...
  const query = { question: 'What is the current status?', typedQuery: {
  ...
  const selected = await ont.verify({ ...query, typedQuery: { ...query.typedQuery, externalId: exact.binding.nativeObject.externalId } });
...
test('canonical name crosses source systems while alias lookup respects explicit scope', async t => {
  const f = fixture(t); f.write(f.record()); const ont = f.open();
  ...
  const aliasWrongScope = await ont.search({ term: 'allocation mismatch', scope: { sourceSystem: 'docs' } });
  assert.equal(aliasWrongScope.state, 'no-construction-match');
  assert.equal(aliasWrongScope.absenceProven, false);
  assert.equal((await ont.search({ term: 'AllocationException', scope: { sourceSystem: 'docs' } })).totalMatches, 1);
  assert.equal((await ont.search({ term: 'allocation' })).state, 'no-construction-match');
  assert.equal((await ont.search({ term: 'What does allocation mismatch mean?' })).state, 'no-construction-match');
});
...
test('same-proposal multi-reviewer agreement does not duplicate passages', async t => {
  const f = fixture(t); const construction = f.compile();
  f.write(f.record(construction)); f.write(f.record(construction, { issuerId: 'reviewer-two' }));
...
test('conflicting same-ID maps are unavailable, never resolved by recency', async t => {
  const f = fixture(t); f.write(f.record());
  const input = clone(f.input); input.claims[0].predicate = 'mentions';
...
test('trust changes require reopen and revoked reviewer yields no construction refs', async t => {
  ...
  assert.equal((await revoked.search({ term: 'AllocationException' })).state, 'unavailable-construction-navigation');
  const query = { question: 'What is the status of CT-17?', typedQuery: {
    sourceSystem: 'clickup', objectType: 'ClickupTask', externalId: 'CT-17', fieldPath: 'status' } };
...
test('references are unforgeable by copying public metadata and cannot cross clients', async t => {
  const f = fixture(t); f.write(f.record()); const a = f.open(); const b = f.open();
  const result = await a.search({ term: 'AllocationException' });
  await assert.rejects(b.read({ ref: result.matches[0].ref }), { code: 'CONSTRUCTION_NAVIGATION_REFERENCE' });
  await assert.rejects(a.read({ ref: 'construction:not-offered' }), { code: 'CONSTRUCTION_NAVIGATION_REFERENCE' });
  assert.throws(() => { result.matches[0].nativeObject.externalId = 'forged'; }, TypeError);
...
test('counts and pagination expose every passage without pulling text into search context', async t => {
  const f = fixture(t, { taskCount: 70 }); f.write(f.record()); const ont = f.open();
  let cursor; const seen = [];
  do {
    const page = await ont.search({ term: 'AllocationException', limit: 20, ...(cursor ? { cursor } : {}) });
...
test('cursors cannot cross clients or queries, and old cursor state is bounded', async t => {
  const f = fixture(t); f.write(f.record()); const ont = f.open();
  const first = await ont.search({ term: 'AllocationException', limit: 1 });
  await assert.rejects(f.open().search({ term: 'AllocationException', cursor: first.nextCursor }), { code: 'CONSTRUCTION_NAVIGATION_CURSOR' });
  await assert.rejects(ont.search({ term: 'allocation mismatch', cursor: first.nextCursor }), { code: 'CONSTRUCTION_NAVIGATION_CURSOR_STALE' });
  for (let i = 0; i < 130; i++) await ont.search({ term: 'AllocationException', limit: 1 });
  await assert.rejects(ont.search({ term: 'AllocationException', cursor: first.nextCursor }), { code: 'CONSTRUCTION_NAVIGATION_CURSOR' });
});
...
test('offered references have a bounded FIFO lifetime independent of source eligibility', async t => {
  ...
  let latest;
  for (let i = 0; i < 16; i++) latest = await ont.search({ term: 'AllocationException', limit: 64 });
  await assert.rejects(ont.read({ ref: first.matches[0].ref }), { code: 'CONSTRUCTION_NAVIGATION_REFERENCE' });
...
test('more than 64 concepts remain ambiguous and fully pageable with bounded summaries', async t => {
  ...
  for (const [start, count] of [[0, 64], [64, 1]]) {
    ...
    input.objectDefs = Array.from({ length: count }, (_, index) => ({ ...clone(f.input.objectDefs[0]), id: `concept-${start + index}` }));
    input.claims = [];
    f.write(f.record(f.compile(input)));
  }
  const ont = f.open();
...
test('ordinary search/read/verify and lifecycle hooks retain their factual path', async t => {
  const f = fixture(t); f.write(f.record()); let searches = 0; let reads = 0;
  const ont = f.open(f.configuration, () => ({ beginSearch: () => { searches++; }, readEvidence: async () => { reads++; return {}; } }));
  const query = { question: 'What is the status of CT-17?', typedQuery: {
    sourceSystem: 'clickup', objectType: 'ClickupTask', externalId: 'CT-17', fieldPath: 'status' } };
  ...
  assert.equal(searches, 2); assert.equal(reads, 2);
...
test('protected history rewind invalidates reads and explicit recovery restores accepted navigation', async t => {
  const f = fixture(t, { protectedHistory: true }); const original = f.record(); f.write(original);
  const old = f.state.store.readRefMetadata(f.route);
  const input = clone(f.input); input.claims = input.claims.slice(0, 2);
  f.write(f.record(f.compile(input), { day: 4, targets: [original.recordSha256] }));
  const ont = f.open(); const offered = await ont.search({ term: 'AllocationException' });
  const current = f.state.store.readRefMetadata(f.route);
  f.backend.compareAndSwap(current.key, { expectedVersion: current.version, bytes: Buffer.from(kernel.stableObjectText(old.ref)) });
  await assert.rejects(ont.read({ ref: offered.matches[0].ref }), { code: 'CONSTRUCTION_NAVIGATION_REFERENCE_INELIGIBLE' });
  assert.equal((await ont.search({ term: 'AllocationException' })).state, 'unavailable-construction-navigation');
...
test('new source clients do not inherit prior-cut maps while an existing client remains explicitly pinned', async t => {
  const f = fixture(t); f.write(f.record()); const pinned = f.open();
...
for (const [name, input] of [
  ...
  ['object type without source', { term: 'AllocationException', scope: { objectType: 'ClickupTask' } }],
  ['unknown key', { term: 'AllocationException', predicate: 'sameAs' }],
  ['oversized page', { term: 'AllocationException', limit: 65 }], ['zero page', { term: 'AllocationException', limit: 0 }],
  ['fractional page', { term: 'AllocationException', limit: 1.5 }], ['bad concept ID', { term: 'AllocationException', conceptId: '../other' }],
