...
import {
  buildSourceNativeProduct, compileSourceNativeSemanticConstruction,
  objectBytesSha256, openProductState, rebindSourceNativeSemanticConstruction,
  stableObjectSha256, stableObjectText, validateSourceNativeSemanticConstruction,
} from '../dist/src/kernel.mjs';

const GUIDE = 'AllocationException means inventory was allocated inconsistently. '
...
function fixture(t, { namespace = 'example', guide = GUIDE, splitGuide = false } = {}) {
  ...
  const sources = [
    ...
    { relativePath: 'docs/note.txt', sourceType: 'docs',
  ...
  const nativeObjectInputs = sources.map((source, index) => ({
    ...
    objectIdentity: { home: 'ObjectDef/InstanceRef', sourceSystem: source.sourceType,
      objectType: index === 1 ? 'ClickupTask' : 'Document', namespace,
  ...
  if (splitGuide) nativeObjectInputs.push({
    relativePath: 'docs/guide.txt',
    objectIdentity: { home: 'ObjectDef/InstanceRef', sourceSystem: 'docs',
      objectType: 'Document', namespace, externalId: 'doc-tail' },
    fields: [{ fieldPath: 'body', value: 'UTF-8: café.' }],
  });
  const buildInput = {
    schemaVersion: 1, kind: 'OpenOntologySourceNativeBuildInputV1',
    ontId: 'construction-example', namespace,
    querySchemas: [
  ...
  function witness(path, quote) {
    ...
    return { nativeObjectSha256: object.nativeObjectSha256, evidence: {
      sourceRef: path, sourceSha256: object.sourceSha256,
      byteStart, byteEnd: byteStart + Buffer.byteLength(value),
      textSha256: objectBytesSha256(Buffer.from(value)),
    } };
  }
  const input = {
    proposedBy: 'fixture-constructor', proposedAt: '2026-09-02T00:00:00.000Z', method: 'authored',
    objectDefs: [{ kind: 'ObjectDef', id: 'allocation-exception', name: 'AllocationException',
      source: witness('docs/guide.txt'), aliases: [{ value: 'allocation mismatch',
        sourceSystem: 'clickup', source: witness('docs/guide.txt') }] }],
    claims: [
      { kind: 'Claim', id: 'definition', about: 'allocation-exception', predicate: 'defines',
        source: witness('docs/guide.txt') },
      { kind: 'Claim', id: 'task-mention', about: 'allocation-exception', predicate: 'mentions',
  ...
  return { root, options, state, input, witness, buildInput,
...
test('compiles scoped ObjectDefs and source attachments without granting semantic authority', (t) => {
  ...
  assert.deepEqual(record.claims.map((claim) => claim.predicate), ['defines', 'mentions']);
  ...
  for (const field of ['answerable', 'proofClosed', 'proofDisposition', 'queryBinding',
    'proofAuthorityProjection', 'proofSufficiencyContract']) assert.equal(field in record, false);
  ...
  assert.throws(() => { record.claims[0].predicate = 'qualifies'; }, TypeError);
...
test('canonical ordering and cold structural/source validation preserve the proposal identity', (t) => {
  ...
  reordered.objectDefs.reverse(); reordered.claims.reverse(); reordered.coverage.reverse();
  ...
  assert.deepEqual(rebindSourceNativeSemanticConstruction({ options: f.options, construction: cold }), record);
...
test('a proposal round-trips as an immutable blob without advancing source or knowledge refs', (t) => {
  const f = fixture(t);
  const record = f.compile();
  const refBefore = f.state.store.readRefMetadata({ ontId: 'construction-example', branch: 'main' });
  const blob = f.state.store.putBlob({ logicalPath: 'blobs/proposals/construction.json',
    bytes: Buffer.from(stableObjectText(record)), mediaType: 'application/json' });
  const cold = openProductState(f.options);
  assert.deepEqual(validateSourceNativeSemanticConstruction(
    JSON.parse(cold.store.readBlob(blob).bytes.toString('utf8'))), record);
  assert.deepEqual(cold.store.readRefMetadata({ ontId: 'construction-example', branch: 'main' }), refBefore);
...
test('unexamined, unsupported and failed source dispositions stay distinct', (t) => {
  ...
  input.objectDefs = []; input.claims = [];
  input.coverage = [
    { ...input.coverage[0], disposition: 'unsupported' },
    { ...input.coverage[1], disposition: 'failed' },
  ];
  const record = f.compile(input);
  assert.deepEqual([record.coverage.examinedSourceCount, record.coverage.unsupportedSourceCount,
    record.coverage.failedSourceCount, record.coverage.unexaminedSourceCount], [0, 1, 1, 1]);
  assert.equal(record.reviewRequired, true);
  assert.equal('complete' in record.coverage, false);
});

const inputFailures = [
  ['invented question', (x) => { x.question = 'What should the answer be?'; }, 'SHAPE'],
  ['caller source binding', (x) => { x.sourceBinding = {}; }, 'SHAPE'],
  ['new Concept kind', (x) => { x.objectDefs[0].kind = 'Concept'; }, 'KIND'],
  ['new attachment kind', (x) => { x.claims[0].kind = 'LinkDef'; }, 'KIND'],
  ['proof predicate', (x) => { x.claims[0].predicate = 'qualifies'; }, 'PREDICATE'],
  ['missing endpoint', (x) => { x.claims[0].about = 'absent'; }, 'ENDPOINT'],
  ['duplicate element ID', (x) => { x.objectDefs.push(clone(x.objectDefs[0])); }, 'DUPLICATE'],
  ['duplicate claim ID', (x) => { x.claims.push(clone(x.claims[0])); }, 'DUPLICATE'],
  ['claim/type ID collision', (x) => { x.claims[0].id = x.objectDefs[0].id; }, 'DUPLICATE'],
  ['duplicate alias', (x) => { x.objectDefs[0].aliases.push(clone(x.objectDefs[0].aliases[0])); }, 'DUPLICATE'],
  ['duplicate source disposition', (x) => { x.coverage.push(clone(x.coverage[0])); }, 'DUPLICATE'],
  ['unexamined evidence', (x) => { x.coverage = []; }, 'COVERAGE'],
  ['failed evidence', (x) => { x.coverage.forEach((r) => { r.disposition = 'failed'; }); }, 'COVERAGE'],
  ['false source disposition', (x) => { x.coverage[0].sourceSha256 = digest; }, 'COVERAGE'],
  ['unknown disposition', (x) => { x.coverage[0].disposition = 'complete'; }, 'COVERAGE'],
  ['unsupported alias', (x) => { x.objectDefs[0].aliases[0].value = 'invented synonym'; }, 'NAME'],
  ['unknown alias system', (x) => { x.objectDefs[0].aliases[0].sourceSystem = 'unmapped'; }, 'SCOPE'],
  ['wrong alias system', (x) => { x.objectDefs[0].aliases[0].sourceSystem = 'docs'; }, 'ATTACHMENT'],
  ['unsupported preferred name', (x) => { x.objectDefs[0].name = 'DifferentConcept'; }, 'NAME'],
  ['unknown native object', (x) => { x.claims[0].source.nativeObjectSha256 = digest; }, 'SOURCE'],
  ['different native object', (x) => { x.claims[0].source.nativeObjectSha256 = x.claims[1].source.nativeObjectSha256; }, 'SOURCE'],
  ['wrong source path', (x) => { x.claims[0].source.evidence.sourceRef = 'unknown.txt'; }, 'SOURCE'],
  ['wrong source hash', (x) => { x.claims[0].source.evidence.sourceSha256 = digest; }, 'SOURCE'],
  ['wrong text hash', (x) => { x.claims[0].source.evidence.textSha256 = digest; }, 'EVIDENCE'],
  ['negative offset', (x) => { x.claims[0].source.evidence.byteStart = -1; }, 'COUNT'],
  ['empty span', (x) => { x.claims[0].source.evidence.byteEnd = 0; }, 'SPAN'],
  ['oversized span', (x) => { x.claims[0].source.evidence.byteEnd = 65537; }, 'SPAN'],
  ['out-of-source span', (x) => { x.claims[0].source.evidence.byteEnd = 1000; }, 'SOURCE'],
  ['noncanonical timestamp', (x) => { x.proposedAt = '2026-09-02'; }, 'TIME'],
  ['unknown method', (x) => { x.method = 'inferred-by-magic'; }, 'METHOD'],
  ['lone surrogate label', (x) => { x.objectDefs[0].name = '\ud800'; }, 'TEXT'],
  ['sparse definitions', (x) => { x.objectDefs = Array(1); }, 'SHAPE'],
];

test('compiler rejects malformed or unsupported inputs instead of repairing their meaning', async (t) => {
  const f = fixture(t);
  for (const [name, change, code] of inputFailures) await t.test(name, () => {
...
test('one native object cannot borrow another object\'s span from the same source file', (t) => {
  ...
  const owner = f.state.objectOnt.map.nativeObjects.find((object) => object.objectIdentity.externalId === 'doc-tail');
...
test('scope and every cut identity are rebound, not trusted from a rehashed record', (t) => {
  ...
  for (const key of Object.keys(record.sourceBinding)) {
    ...
    rehash(changed);
    assert.throws(() => rebindSourceNativeSemanticConstruction({ options: f.options,
      construction: changed }), { code: 'SEMANTIC_CONSTRUCTION_BINDING' }, key);
  }
  const changed = clone(record);
  changed.coverage.sourceCount += 1; changed.coverage.unexaminedSourceCount += 1;
  assert.throws(() => rebindSourceNativeSemanticConstruction({ options: f.options,
    construction: rehash(changed) }), { code: 'SEMANTIC_CONSTRUCTION_BINDING' });
});
...
test('even identical source text in another namespace cannot receive an old construction', (t) => {
  ...
  assert.throws(() => rebindSourceNativeSemanticConstruction({ options: other.options,
    construction: f.compile() }), { code: 'SEMANTIC_CONSTRUCTION_BINDING' });
});
...
test('a real source advance preserves proposal bytes but invalidates their current-cut binding', (t) => {
  ...
  assert.notEqual(openProductState(nextOptions).objectOnt.commitSha256, record.sourceBinding.sourceCommitSha256);
  assert.throws(() => rebindSourceNativeSemanticConstruction({ options: nextOptions, construction: record }),
    { code: 'SEMANTIC_CONSTRUCTION_BINDING' });
  assert.throws(() => rebindSourceNativeSemanticConstruction({ options: f.options, construction: record }),
    { code: 'SOURCE_NATIVE_PRODUCT_REF' });
  assert.equal(stableObjectText(validateSourceNativeSemanticConstruction(JSON.parse(bytes))), bytes);
});

test('structural validation rejects altered flags, counts, hashes and hidden fields', (t) => {
  const record = fixture(t).compile();
  for (const mutate of [
    (x) => { x.reviewRequired = false; }, (x) => { x.navigationOnly = false; },
    (x) => { x.exactSourcesRemainAuthority = false; }, (x) => { x.schemaVersion = 2; },
    (x) => { x.coverage.examinedSourceCount = 0; }, (x) => { x.kind = 'Claim'; },
  ]) {
  ...
  assert.throws(() => validateSourceNativeSemanticConstruction({ ...record, constructionSha256: digest }),
    { code: 'SEMANTIC_CONSTRUCTION_RECORD' });
});
...
test('record collection and passage limits refuse overflow without truncating', (t) => {
  ...
  many.objectDefs = Array.from({ length: 64 }, (_, i) => ({ ...clone(many.objectDefs[0]), id: `concept-${i}` }));
  many.claims = Array.from({ length: 256 }, (_, i) => ({ ...clone(many.claims[0]), id: `claim-${i}`, about: 'concept-0' }));
  assert.equal(f.compile(many).claims.length, 256);
  assert.throws(() => f.compile({ ...many, objectDefs: [...many.objectDefs, clone(many.objectDefs[0])] }),
  ...
  aliases.objectDefs[0].aliases = Array.from({ length: 17 }, () => clone(f.input.objectDefs[0].aliases[0]));
...
test('a structurally valid record accepts exactly 1 MiB and refuses one byte more', (t) => {
  ...
  record.objectDefs = Array.from({ length: 64 }, (_, i) => ({ ...clone(record.objectDefs[0]),
    id: `concept-${String(i).padStart(3, '0')}`, aliases: Array.from({ length: 16 }, (_, j) => ({
      ...clone(record.objectDefs[0].aliases[0]), value: `alias-${String(j).padStart(3, '0')}`,
    })) }));
  record.claims = [];
  const witnesses = record.objectDefs.flatMap((object) => [object.source, ...object.aliases.map((alias) => alias.source)]);
...
test('512 source dispositions are structurally allowed but cannot invent source-catalog coverage', (t) => {
  ...
  record.objectDefs = []; record.claims = [];
  record.coverage = { sourceCount: 512, examinedSourceCount: 0, failedSourceCount: 0,
    unsupportedSourceCount: 512, unexaminedSourceCount: 0,
    sourceResults: Array.from({ length: 512 }, (_, i) => ({
      sourceRef: `source-${String(i).padStart(3, '0')}`, sourceSha256: digest,
      disposition: 'unsupported',
    })) };
  assert.equal(validateSourceNativeSemanticConstruction(rehash(record)).coverage.sourceResults.length, 512);
  assert.throws(() => rebindSourceNativeSemanticConstruction({ options: f.options, construction: record }),
    { code: 'SEMANTIC_CONSTRUCTION_BINDING' });
});
