...
const unexpectedRoots = [...new Set(tracked.map((path) => path.split('/')[0]))].filter(
...
if (missingTracked.length)
  fail(`tracked paths missing from checkout: ${missingTracked.slice(0, 10).join(', ')}`);
...
for (const path of tracked) {
  ...
  const secretPatterns = [
    ...
    [
      ...
      /\b(?:postgres(?:ql)?|mysql|mongodb(?:\+srv)?):\/\/[^/\s:@]+:[^@\s/]+@/u,
    ],
  ];
  ...
  if (
    path.endsWith('.svg') &&
    /<script\b|\bon[a-z]+\s*=|(?:href|xlink:href)\s*=\s*["'](?:https?:|data:)/iu.test(text)
  ) {
...
try {
  ...
  execFileSync('tar', ['-xzf', join(packageSandbox, packed.filename), '-C', packageSandbox], {
    cwd: root,
  });
} catch (error) {
...
const required = [
  ...
  'dist/cli/resolver.d.ts',
  'dist/kernel.js',
  'dist/kernel.d.ts',
  'dist/openontology.js',
  'dist/openontology.d.ts',
  'examples/quickstart/source-native-input.json',
...
const forbiddenPacked = packedPaths.filter(
  (path) =>
...
const invalidGenerated = packedPaths.filter(
  (path) => path.startsWith('dist/') && !/\.(?:js|d\.ts)(?:\.map)?$/u.test(path),
);
if (invalidGenerated.length)
  fail(`unexpected generated package files: ${invalidGenerated.join(', ')}`);
const packageJsonInTarball = JSON.parse(
  readFileSync(join(packageSandbox, 'package', 'package.json'), 'utf8'),
);
...
for (const path of packedPaths.filter((name) => name.endsWith('.map'))) {
  let map;
  try {
    map = JSON.parse(readFileSync(join(packageSandbox, 'package', path), 'utf8'));
  } catch {
  ...
  if (
    ...
    (hasInlineSources &&
      (!Array.isArray(map.sourcesContent) || map.sources.length !== map.sourcesContent.length)) ||
    map.sources.some(
      (source) =>
...
process.stdout.write(
  `public release check passed: ${tracked.length} tracked files, ${packed.entryCount} package files\n`,
);
